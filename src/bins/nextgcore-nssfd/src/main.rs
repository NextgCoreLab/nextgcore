...
async fn handle_ns_selection(request: &SbiRequest) -> SbiResponse {
    ...
    let nf_type = request
        ...
        .map(|s| s.as_str())
        .unwrap_or("AMF");
    let nf_id = request.http.params.get("nf-id").map(|s| s.as_str());

    log::info!("NS Selection: nf-type={nf_type}, nf-id={nf_id:?}");

    // Parse slice-info-request-for-pdu-session (JSON query param)
    let slice_info_json = request
        .http
        .params
        .get("slice-info-request-for-pdu-session")
    ...
    if let Some(tai_str) = request.http.params.get("tai") {
        if let Ok(tai_json) = serde_json::from_str::<serde_json::Value>(tai_str) {
            let plmn_mcc = tai_json
                .get("plmnId")
                .and_then(|p| p.get("mcc"))
                .and_then(|v| v.as_str())
                .unwrap_or("000");
            let plmn_mnc = tai_json
                .get("plmnId")
                ...
                match ogs_dbi::ogs_dbi_subscription_data(_supi) {
                    Ok(sub_data) => {
                        ...
                        if !subscribed.is_empty() {
                            log::info!(
                                "Filtering allowed NSSAIs against {} subscribed slices for SUPI",
                                subscribed.len()
                            );
                            ...
                            log::debug!(
                                "After subscription filtering: {} allowed S-NSSAIs",
                                allowed_snssai_list.len()
                            );
                        }
                    }
                    Err(e) => {
...
async fn handle_nssai_availability_update(nf_id: &str, request: &SbiRequest) -> SbiResponse {
    ...
    if let Some(infos) = availability_data
        .get("nssaiAvailabilityInfos")
    ...
    log::info!(
        "Stored NSSAI availability for NF {}: {} S-NSSAIs",
        nf_id,
        supported_snssai_list.len()
    );
...
async fn handle_nssai_availability_patch(nf_id: &str, request: &SbiRequest) -> SbiResponse {
    ...
    let ctx = nssf_self();
    if let Ok(context) = ctx.read() {
        let mut avail = context.get_nssai_availability(nf_id).unwrap_or_else(|| {
            context::NssaiAvailabilityInfo {
                nf_id: nf_id.to_string(),
                ...
                match op {
                    "add" | "replace" => {
                        if let Some(value) = patch.get("value") {
                            if let Some(snssai_list) =
                                value.get("supportedNssaiList").and_then(|v| v.as_array())
                            {
                                ...
                                for snssai_json in snssai_list {
                                    ...
                                    avail
                                        ...
                                        .push(context::SNssai::new(sst, sd));
                                }
                            }
                        }
                    }
...
async fn handle_nssai_availability_options() -> SbiResponse {
    ...
    SbiResponse::with_status(200).with_header("Allow", "GET, PUT, PATCH, DELETE, OPTIONS")
}
...
async fn handle_subscription_create(request: &SbiRequest) -> SbiResponse {
    ...
    SbiResponse::with_status(201)
        .with_header(
            "Location",
            format!("/nnssf-nssaiavailability/v1/subscriptions/{subscription_id}"),
        )
        .with_json_body(&serde_json::json!({
