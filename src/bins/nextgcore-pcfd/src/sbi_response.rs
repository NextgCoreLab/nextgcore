...
//!
//! Helper functions for sending SBI error responses in the PCF state machines.
...
/// Send a USER_UNKNOWN error response (3GPP specific)
#[allow(dead_code)]
pub fn send_user_unknown_response(stream_id: u64) {
    send_error_response(stream_id, 404, "USER_UNKNOWN");
}

/// Send a POLICY_CONTEXT_DENIED error response (3GPP specific)
#[allow(dead_code)]
pub fn send_policy_context_denied_response(stream_id: u64) {
    send_error_response(stream_id, 403, "POLICY_CONTEXT_DENIED");
}
...
mod tests {
    ...
    static NEXT_STREAM_ID: AtomicU64 = AtomicU64::new(100000);
    ...
    fn test_send_error_response_404() {
        let stream_id = next_stream_id();
        send_error_response(stream_id, 404, "Test not found");
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
    ...
    #[test]
    fn test_send_user_unknown_response() {
        let stream_id = next_stream_id();
    ...
    #[test]
    fn test_send_policy_context_denied_response() {
        let stream_id = next_stream_id();
        send_policy_context_denied_response(stream_id);
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
