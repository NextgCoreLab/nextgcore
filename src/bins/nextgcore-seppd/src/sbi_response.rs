...
//!
//! Helper functions for sending SBI error responses in the SEPP state machines.

use nextgcore_sbi::message::SbiResponse;
use nextgcore_sbi::server::{
    send_bad_request, send_error, send_forbidden, send_gateway_timeout, send_method_not_allowed,
    send_not_found,
};
...
pub fn send_error_response(stream_id: u64, status: u16, detail: &str) {
    let response = match status {
        ...
        501 => send_error(501, "Not Implemented", detail, Some("NOT_IMPLEMENTED")),
...
#[allow(dead_code)]
pub fn send_not_implemented_response(stream_id: u64, detail: &str) {
    send_error_response(stream_id, 501, detail);
}
...
mod tests {
    ...
    static NEXT_STREAM_ID: AtomicU64 = AtomicU64::new(400000);
    ...
    fn test_send_error_response_400() {
        ...
        send_error_response(stream_id, 400, "Test bad request");
    ...
    #[test]
    fn test_send_error_response_501() {
        let stream_id = next_stream_id();
        send_error_response(stream_id, 501, "Test not implemented");
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
        assert_eq!(response.unwrap().status, 501);
    }

    #[test]
    fn test_send_error_response_504() {
        let stream_id = next_stream_id();
        send_error_response(stream_id, 504, "Test gateway timeout");
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
    ...
    fn test_send_gateway_timeout_response() {
        let stream_id = next_stream_id();
        send_gateway_timeout_response(stream_id, "Timeout waiting for response");
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
        assert_eq!(response.unwrap().status, 504);
    }
    ...
    #[test]
    fn test_send_not_implemented_response() {
        let stream_id = next_stream_id();
        send_not_implemented_response(stream_id, "PRINS is not supported");
        let response = take_pending_response(stream_id);
        assert!(response.is_some());
        assert_eq!(response.unwrap().status, 501);
    }
