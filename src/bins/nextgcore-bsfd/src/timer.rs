//! BSF Timer Management
//!
//! Timer management for BSF operations using the shared AsyncTimerMgr
//! from nextgcore-core.

use crate::event::BsfTimerId;
use nextgcore_core::async_timer::{AsyncTimerMgr, TimerMode};
use std::sync::Arc;
use std::time::Duration;

/// BSF Timer Manager wrapping the generic AsyncTimerMgr
pub struct BsfTimerManager {
    inner: AsyncTimerMgr<BsfTimerId>,
}

impl BsfTimerManager {
    /// Create a new BSF timer manager
    pub fn new() -> Self {
        Self {
    ...
    pub fn start(&self, timer_type: BsfTimerId, duration: Duration, data: Option<String>) -> u64 {
        self.inner
    ...
    pub fn process_expired(&self) -> Vec<nextgcore_core::async_timer::AsyncTimerEntry<BsfTimerId>> {
    ...
    pub fn inner(&self) -> &AsyncTimerMgr<BsfTimerId> {
        &self.inner
    }
}

impl Default for BsfTimerManager {
    fn default() -> Self {
        Self::new()
    }
}
...
static TIMER_MANAGER: std::sync::OnceLock<Arc<BsfTimerManager>> = std::sync::OnceLock::new();

/// Get the global BSF timer manager
pub fn timer_manager() -> Arc<BsfTimerManager> {
    TIMER_MANAGER
        .get_or_init(|| Arc::new(BsfTimerManager::new()))
        .clone()
}

/// Get timer name for logging
#[allow(dead_code)]
pub fn bsf_timer_get_name(timer_id: BsfTimerId) -> &'static str {
    timer_id.name()
}

/// Default timer durations for BSF
#[allow(dead_code)]
pub mod defaults {
    use std::time::Duration;
...
mod tests {
    ...
    fn test_timer_manager_process_expired() {
        ...
        mgr.start(BsfTimerId::SbiClientWait, Duration::from_millis(10), None);

        thread::sleep(Duration::from_millis(20));

        let expired = mgr.process_expired();
        assert_eq!(expired.len(), 1);
        assert_eq!(expired[0].timer_type, BsfTimerId::SbiClientWait);
