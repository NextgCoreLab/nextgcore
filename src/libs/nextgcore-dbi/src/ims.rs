...
pub struct NextgcoreMsisdnData {
    pub imsi: NextgcoreMsisdn,
    pub msisdn: Vec<NextgcoreMsisdn>,
    pub num_of_msisdn: usize,
}

impl NextgcoreMsisdnData {
    pub fn new() -> Self {
        Self::default()
    }
}
...
///
/// # Returns
/// * `Ok(NextgcoreMsisdnData)` with IMSI and MSISDN data
/// * `Err(DbiError)` on failure
pub fn nextgcore_dbi_msisdn_data(imsi_or_msisdn_bcd: &str) -> DbiResult<NextgcoreMsisdnData> {
    ...
    let mut msisdn_data = NextgcoreMsisdnData::new();

    // Parse IMSI
    if let Ok(imsi_str) = document.get_str("imsi") {
        msisdn_data.imsi.bcd = imsi_str.to_string();
        nextgcore_bcd_to_buffer(&msisdn_data.imsi.bcd, &mut msisdn_data.imsi.buf);
    ...
    if let Ok(msisdn_array) = document.get_array("msisdn") {
        for msisdn_val in msisdn_array {
            if msisdn_data.num_of_msisdn >= NEXTGCORE_MAX_NUM_OF_MSISDN {
                break;
            }
...
pub fn nextgcore_dbi_ims_data(supi: &str) -> DbiResult<NextgcoreImsData> {
    ...
    let mut ims_data = NextgcoreImsData::new();

    // Parse MSISDN array
    if let Ok(msisdn_array) = document.get_array("msisdn") {
        for msisdn_val in msisdn_array {
            if ims_data.num_of_msisdn >= NEXTGCORE_MAX_NUM_OF_MSISDN {
                break;
            }
            if let Bson::String(bcd) = msisdn_val {
                let mut msisdn = NextgcoreMsisdn::default();
    ...
    if let Ok(ifc_array) = document.get_array("ifc") {
        for ifc_val in ifc_array {
            if ims_data.num_of_ifc >= NEXTGCORE_MAX_NUM_OF_IFC {
                break;
            }
...
fn parse_ifc(doc: &Document) -> NextgcoreIfc {
    let mut ifc = NextgcoreIfc::default();
...
fn parse_trigger_point(doc: &Document) -> NextgcoreTriggerPoint {
    let mut trigger_point = NextgcoreTriggerPoint::default();
    ...
    if let Ok(spt_array) = doc.get_array("spt") {
        for spt_val in spt_array {
            if trigger_point.num_of_spt >= NEXTGCORE_MAX_NUM_OF_SPT {
                break;
            }
...
fn parse_spt(doc: &Document) -> NextgcoreSpt {
    let mut spt = NextgcoreSpt::default();
    ...
    // Parse method (sets type to NEXTGCORE_SPT_HAS_METHOD)
    if let Ok(method) = doc.get_str("method") {
        spt.method = Some(method.to_string());
        spt.spt_type = NEXTGCORE_SPT_HAS_METHOD;
    }

    // Parse session case (sets type to NEXTGCORE_SPT_HAS_SESSION_CASE)
    if let Ok(session_case) = doc.get_i32("session_case") {
        spt.session_case = session_case;
        spt.spt_type = NEXTGCORE_SPT_HAS_SESSION_CASE;
    }

    // Parse SIP header (sets type to NEXTGCORE_SPT_HAS_SIP_HEADER)
    if let Ok(sip_header_doc) = doc.get_document("sip_header") {
        ...
        spt.spt_type = NEXTGCORE_SPT_HAS_SIP_HEADER;
    }

    // Parse SDP line (sets type to NEXTGCORE_SPT_HAS_SDP_LINE)
    if let Ok(sdp_line_doc) = doc.get_document("sdp_line") {
        ...
        spt.spt_type = NEXTGCORE_SPT_HAS_SDP_LINE;
    }

    // Parse request URI (sets type to NEXTGCORE_SPT_HAS_REQUEST_URI)
    if let Ok(request_uri) = doc.get_str("request_uri") {
        spt.request_uri = Some(request_uri.to_string());
        spt.spt_type = NEXTGCORE_SPT_HAS_REQUEST_URI;
    }
