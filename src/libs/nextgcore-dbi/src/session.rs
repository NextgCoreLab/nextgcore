...
///
/// # Returns
/// * `Ok(NextgcoreSessionData)` with session and PCC rules
/// * `Err(DbiError)` on failure
pub fn nextgcore_dbi_session_data(
    supi: &str,
    s_nssai: Option<&NextgcoreSNssai>,
    dnn: &str,
) -> DbiResult<NextgcoreSessionData> {
    ...
    let slice_array = document
        .get_array(NEXTGCORE_SLICE_STRING)
        .map_err(|_| DbiError::FieldNotFound(NEXTGCORE_SLICE_STRING.to_string()))?;

    for slice_val in slice_array {
        if let Bson::Document(slice_doc) = slice_val {
            // Check SST presence
            let sst = match slice_doc.get_i32(NEXTGCORE_SST_STRING) {
                Ok(s) => s as u8,
                Err(_) => {
                    log::error!("No SST");
                    continue;
            ...
            // Parse SD
            let sd = if let Ok(sd_str) = slice_doc.get_str(NEXTGCORE_SD_STRING) {
                NextgcoreUint24::from_hex_string(sd_str)
                    .unwrap_or(NextgcoreUint24::new(NEXTGCORE_S_NSSAI_NO_SD_VALUE))
            } else {
                NextgcoreUint24::new(NEXTGCORE_S_NSSAI_NO_SD_VALUE)
            };
            ...
            if let Some(req_snssai) = s_nssai {
                ...
                if req_snssai.sd.v != NEXTGCORE_S_NSSAI_NO_SD_VALUE
                    && sd.v != NEXTGCORE_S_NSSAI_NO_SD_VALUE
            ...
            if let Ok(session_array) = slice_doc.get_array(NEXTGCORE_SESSION_STRING) {
                for session_val in session_array {
                    if let Bson::Document(session_doc) = session_val {
                        if let Ok(name) = session_doc.get_str(NEXTGCORE_NAME_STRING) {
...
fn parse_session_data(doc: &Document, dnn: &str) -> DbiResult<NextgcoreSessionData> {
    let mut session_data = NextgcoreSessionData::new();
    ...
    if let Ok(pcc_rule_array) = doc.get_array(NEXTGCORE_PCC_RULE_STRING) {
        ...
        for pcc_rule_val in pcc_rule_array {
            if pcc_rule_index >= NEXTGCORE_MAX_NUM_OF_PCC_RULE {
                break;
            }
...
fn parse_qos(doc: &Document) -> NextgcoreQos {
    ...
    // Parse MBR
    if let Ok(mbr_doc) = doc.get_document(NEXTGCORE_MBR_STRING) {
        qos.mbr = parse_ambr(mbr_doc);
    }

    // Parse GBR
    if let Ok(gbr_doc) = doc.get_document(NEXTGCORE_GBR_STRING) {
...
fn parse_pcc_rule(doc: &Document) -> NextgcorePccRule {
    let mut pcc_rule = NextgcorePccRule::default();
    ...
    // Parse flows
    if let Ok(flow_array) = doc.get_array(NEXTGCORE_FLOW_STRING) {
        for flow_val in flow_array {
            if pcc_rule.num_of_flow >= NEXTGCORE_MAX_NUM_OF_FLOW_IN_PCC_RULE {
                break;
            }

            if let Bson::Document(flow_doc) = flow_val {
                let mut flow = NextgcoreFlow::default();

                if let Ok(direction) = flow_doc.get_i32(NEXTGCORE_DIRECTION_STRING) {
                    flow.direction = direction;
                }

                if let Ok(description) = flow_doc.get_str(NEXTGCORE_DESCRIPTION_STRING) {
...
fn parse_pcc_rule_qos(doc: &Document) -> NextgcoreQos {
    let mut qos = NextgcoreQos::default();
