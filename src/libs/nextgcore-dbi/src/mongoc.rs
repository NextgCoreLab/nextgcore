...
//! Ported from lib/dbi/nextgcore-mongoc.c in the C implementation.
...
#[derive(Debug, Default)]
pub struct NextgcoreDbi {
    pub mongoc: NextgcoreMongoc,
    pub subscriber_collection: Option<Collection<Document>>,
}

/// Global singleton for database interface
static NEXTGCORE_DBI: OnceLock<Arc<Mutex<NextgcoreDbi>>> = OnceLock::new();

/// Get the global database interface instance
pub fn nextgcore_mongoc() -> Arc<Mutex<NextgcoreDbi>> {
    NEXTGCORE_DBI
        .get_or_init(|| Arc::new(Mutex::new(NextgcoreDbi::default())))
        .clone()
}
...
pub fn nextgcore_mongoc_init(db_uri: &str) -> DbiResult<()> {
    if db_uri.is_empty() {
        log::error!("No DB_URI");
...
/// Async wrapper for `nextgcore_dbi_init` — offloads the blocking MongoDB
...
pub async fn nextgcore_dbi_init_async(db_uri: String) -> DbiResult<()> {
    tokio::task::spawn_blocking(move || nextgcore_dbi_init(&db_uri))
        .await
        .unwrap_or_else(|e| {
...
/// [`nextgcore_dbi_init_async`] instead when running inside a tokio async context.
pub fn nextgcore_dbi_init(db_uri: &str) -> DbiResult<()> {
    nextgcore_mongoc_init(db_uri)?;
...
pub fn nextgcore_dbi_final() {
    let dbi = nextgcore_mongoc();
    {
        let mut dbi_guard = dbi.lock().unwrap();
        dbi_guard.subscriber_collection = None;
    }
    nextgcore_mongoc_final();
}
...
impl NextgcoreDbi {
    /// Construct a pre-initialised `NextgcoreDbi` from an existing MongoDB `Client`.
    ///
    ...
    /// let dbi = NextgcoreDbi::with_client(client, "test_db");
    /// ```
...
/// Rust, this replaces the inner `NextgcoreDbi` contents through the Mutex rather
