...
// Strategy for generating NextgcoreUint24 values
fn uint24_strategy() -> impl Strategy<Value = NextgcoreUint24> {
    (0u32..=0xFFFFFFu32).prop_map(NextgcoreUint24::new)
}

// Strategy for generating S-NSSAI values
fn s_nssai_strategy() -> impl Strategy<Value = NextgcoreSNssai> {
    (
        any::<u8>(),
        prop_oneof![Just(None), uint24_strategy().prop_map(Some)],
    )
        .prop_map(|(sst, sd)| NextgcoreSNssai::new(sst, sd.map(|s| s.v)))
}

// Strategy for generating AMBR values
fn ambr_strategy() -> impl Strategy<Value = NextgcoreAmbr> {
    (any::<u64>(), any::<u64>()).prop_map(|(downlink, uplink)| NextgcoreAmbr { downlink, uplink })
}

// Strategy for generating ARP values
fn arp_strategy() -> impl Strategy<Value = NextgcoreArp> {
    (1u8..=15, 0u8..=1, 0u8..=1).prop_map(
        |(priority_level, pre_emption_capability, pre_emption_vulnerability)| NextgcoreArp {
            priority_level,
...
fn qos_strategy() -> impl Strategy<Value = NextgcoreQos> {
    (1u8..=9, arp_strategy(), ambr_strategy(), ambr_strategy()).prop_map(
        |(index, arp, mbr, gbr)| NextgcoreQos {
            index,
            arp,
            mbr,
            gbr,
...
proptest! {
    /// Property 13.1: NextgcoreUint24 round-trip through bytes
    #[test]
    fn test_uint24_byte_roundtrip(value in 0u32..=0xFFFFFFu32) {
        let u = NextgcoreUint24::new(value);
        let bytes = u.to_be_bytes();
        let u2 = NextgcoreUint24::from_be_bytes(bytes);
        prop_assert_eq!(u.v, u2.v);
    }

    /// Property 13.2: NextgcoreUint24 overflow handling
    #[test]
    ...
    fn test_s_nssai_sd_detection(sst in any::<u8>(), sd in prop_oneof![Just(None), (0u32..0xFFFFFEu32).prop_map(Some)]) {
        let snssai = NextgcoreSNssai::new(sst, sd);
        if sd.is_some() && sd.expect("value expected") != NEXTGCORE_S_NSSAI_NO_SD_VALUE {
    ...
    fn test_supi_parsing(id_type in "[a-z]{2,8}", id_value in "[a-zA-Z0-9]{1,20}") {
        ...
        let parsed_type = nextgcore_id_get_type(&supi);
        let parsed_value = nextgcore_id_get_value(&supi);
    ...
    fn test_hex_conversion(hex_str in hex_string_strategy(16)) {
        let mut buf = [0u8; 16];
        let len = nextgcore_ascii_to_hex(&hex_str, &mut buf);
        prop_assert_eq!(len, 16);

        // Verify each byte
        for i in 0..16 {
            let expected = u8::from_str_radix(&hex_str[i*2..i*2+2], 16).expect("value expected");
    ...
    fn test_bcd_conversion(bcd in "[0-9]{1,15}") {
        let mut buf = Vec::new();
        let len = nextgcore_bcd_to_buffer(&bcd, &mut buf);
    ...
    fn test_subscription_data_clear(
        ...
        num_msisdn in 0usize..=NEXTGCORE_MAX_NUM_OF_MSISDN
    ) {
        let mut data = NextgcoreSubscriptionData::new();
        data.imsi = Some(imsi);
    ...
    fn test_session_data_clear(
        ...
        num_pcc_rule in 0usize..=NEXTGCORE_MAX_NUM_OF_PCC_RULE
    ) {
        let mut data = NextgcoreSessionData::new();
    ...
    fn test_ims_data_clear(
        num_msisdn in 0usize..=NEXTGCORE_MAX_NUM_OF_MSISDN,
        num_ifc in 0usize..=NEXTGCORE_MAX_NUM_OF_IFC
    ) {
        let mut data = NextgcoreImsData::new();
        data.num_of_msisdn = num_msisdn;
    ...
    fn test_pcc_rule_clear(
        ...
        name in "[a-zA-Z0-9]{1,20}",
        num_flow in 0usize..=NEXTGCORE_MAX_NUM_OF_FLOW_IN_PCC_RULE
    ) {
        let mut rule = NextgcorePccRule::default();
        rule.id = Some(id);
        rule.name = Some(name);
...
mod unit_tests {
    ...
    fn test_uint24_from_hex_string() {
        let u = NextgcoreUint24::from_hex_string("000001").unwrap();
        assert_eq!(u.v, 1);

        let u2 = NextgcoreUint24::from_hex_string("FFFFFF").unwrap();
        assert_eq!(u2.v, 0xFFFFFF);

        let u3 = NextgcoreUint24::from_hex_string("invalid");
        assert!(u3.is_none());
    }

    #[test]
    fn test_s_nssai_no_sd() {
        let snssai = NextgcoreSNssai::new(1, None);
        assert_eq!(snssai.sst, 1);
        assert_eq!(snssai.sd.v, NEXTGCORE_S_NSSAI_NO_SD_VALUE);
    ...
    fn test_bcd_to_buffer() {
        let mut buf = Vec::new();
        let len = nextgcore_bcd_to_buffer("123456789012345", &mut buf);
        assert_eq!(len, 8);
