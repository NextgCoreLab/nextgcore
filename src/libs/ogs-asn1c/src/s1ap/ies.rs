...
/// MME-UE-S1AP-ID - Unique identifier for UE in MME
/// ASN.1: MME-UE-S1AP-ID ::= INTEGER (0..4294967295)
...
impl AperEncode for MmeUeS1apId {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for MmeUeS1apId {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(MmeUeS1apId(value as u32))
    }
}

/// ENB-UE-S1AP-ID - Unique identifier for UE in eNB
/// ASN.1: ENB-UE-S1AP-ID ::= INTEGER (0..16777215)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct EnbUeS1apId(pub u32);

impl EnbUeS1apId {
    pub const CONSTRAINT: Constraint = Constraint::new(0, 16777215);
}

impl AperEncode for EnbUeS1apId {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for EnbUeS1apId {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(EnbUeS1apId(value as u32))
    }
}
...
/// RelativeMMECapacity - Relative capacity of MME
/// ASN.1: RelativeMMECapacity ::= INTEGER (0..255)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct RelativeMmeCapacity(pub u8);

impl RelativeMmeCapacity {
    pub const CONSTRAINT: Constraint = Constraint::new(0, 255);
}

impl AperEncode for RelativeMmeCapacity {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for RelativeMmeCapacity {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(RelativeMmeCapacity(value as u8))
    }
}
...
impl AperEncode for ERabId {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for ERabId {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(ERabId(value as u8))
    }
}

/// QCI - QoS Class Identifier
/// ASN.1: QCI ::= INTEGER (0..255)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
impl AperEncode for Qci {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for Qci {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(Qci(value as u8))
    }
...
/// PriorityLevel - Priority level for allocation and retention
/// ASN.1: PriorityLevel ::= INTEGER (0..15)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct PriorityLevel(pub u8);

impl PriorityLevel {
    pub const CONSTRAINT: Constraint = Constraint::new(0, 15);
}

impl AperEncode for PriorityLevel {
    fn encode_aper(&self, encoder: &mut AperEncoder) -> PerResult<()> {
        encoder.encode_constrained_whole_number(self.0 as i64, &Self::CONSTRAINT)
    }
}

impl AperDecode for PriorityLevel {
    fn decode_aper(decoder: &mut AperDecoder) -> PerResult<Self> {
        let value = decoder.decode_constrained_whole_number(&Self::CONSTRAINT)?;
        Ok(PriorityLevel(value as u8))
    }
}
