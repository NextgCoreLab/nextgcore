...
impl MetricsContext {
    ...
    pub fn add_server(&mut self, config: ServerConfig) -> &MetricsServer {
        ...
        self.servers.last().expect("value expected")
    }
...
/// Initialize the global metrics context
pub fn nextgcore_metrics_context_init() {
    MetricsContext::init();
}

/// Open the metrics context
pub fn nextgcore_metrics_context_open() {
...
pub fn nextgcore_metrics_context_close() {
    if let Some(ctx) = MetricsContext::get() {
        if let Ok(mut context) = ctx.write() {
            context.close();
        }
...
/// Finalize the metrics context
pub fn nextgcore_metrics_context_final() {
    if let Some(ctx) = MetricsContext::get() {
        if let Ok(mut context) = ctx.write() {
            context.finalize();
        }
...
/// Get the global metrics context
pub fn nextgcore_metrics_self() -> Option<Arc<RwLock<MetricsContext>>> {
    MetricsContext::get()
}
