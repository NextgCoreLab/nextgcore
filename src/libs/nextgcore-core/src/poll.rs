//! Event polling
//!
//! Exact port of lib/core/nextgcore-poll.h and nextgcore-poll.c
...
use crate::sockopt::{nextgcore_closeonexec, nextgcore_nonblocking};
use crate::time::NextgcoreTime;

/// Poll event flags
pub const NEXTGCORE_POLLIN: i16 = 0x01;
pub const NEXTGCORE_POLLOUT: i16 = 0x02;

/// Poll handler function type
pub type NextgcorePollHandler = Box<dyn FnMut(i16, NextgcoreSocket, *mut std::ffi::c_void) + Send>;

/// Poll structure (identical to nextgcore_poll_t)
pub struct NextgcorePoll {
    pub id: usize,
    pub when: i16,
    pub fd: NextgcoreSocket,
    pub handler: NextgcorePollHandler,
    pub data: *mut std::ffi::c_void,
}

// Safety: NextgcorePoll is Send because we manage the raw pointer carefully
unsafe impl Send for NextgcorePoll {}

/// Pollset structure (identical to nextgcore_pollset_t)
pub struct NextgcorePollset {
    polls: HashMap<usize, NextgcorePoll>,
    next_id: AtomicUsize,
    capacity: usize,

    // Notify pipe for waking up poll
    notify_read_fd: NextgcoreSocket,
    notify_write_fd: NextgcoreSocket,
...
impl NextgcorePollset {
    /// Create a new pollset (identical to nextgcore_pollset_create)
    pub fn create(capacity: usize) -> Option<Self> {
        ...
        nextgcore_nonblocking(notify_read_fd);
        nextgcore_closeonexec(notify_read_fd);
        nextgcore_nonblocking(notify_write_fd);
        nextgcore_closeonexec(notify_write_fd);
    ...
    /// Add a poll to the pollset (identical to nextgcore_pollset_add)
    pub fn add<F>(
        &mut self,
        when: i16,
        fd: NextgcoreSocket,
    ...
    where
        F: FnMut(i16, NextgcoreSocket, *mut std::ffi::c_void) + Send + 'static,
    {
        ...
        // Set non-blocking and close-on-exec
        nextgcore_nonblocking(fd);
        nextgcore_closeonexec(fd);
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if (when & NEXTGCORE_POLLIN) != 0 {
                events |= libc::EPOLLIN as u32;
            }
            if (when & NEXTGCORE_POLLOUT) != 0 {
    ...
    /// Remove a poll from the pollset (identical to nextgcore_pollset_remove)
    pub fn remove(&mut self, id: usize) -> i32 {
        if let Some(poll) = self.polls.remove(&id) {
            ...
            #[cfg(target_os = "macos")]
            {
                if (poll.when & NEXTGCORE_POLLIN) != 0 {
                    let event = libc::kevent {
                ...
                if (poll.when & NEXTGCORE_POLLOUT) != 0 {
                    let event = libc::kevent {
                        ident: poll.fd as usize,
                        filter: libc::EVFILT_WRITE,
                        flags: libc::EV_DELETE,
    ...
    /// Poll for events (identical to nextgcore_pollset_poll)
    pub fn poll(&mut self, timeout: NextgcoreTime) -> i32 {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            for i in 0..nfds as usize {
                ...
                if (events[i].events & libc::EPOLLIN as u32) != 0 {
                    when |= NEXTGCORE_POLLIN;
                }
                if (events[i].events & libc::EPOLLOUT as u32) != 0 {
                    when |= NEXTGCORE_POLLOUT;
                }
    ...
    /// Notify the pollset to wake up (identical to nextgcore_pollset_notify)
...
impl Drop for NextgcorePollset {
    fn drop(&mut self) {
        #[cfg(target_os = "linux")]
        unsafe {
...
pub fn nextgcore_pollset_create(capacity: usize) -> Option<NextgcorePollset> {
    NextgcorePollset::create(capacity)
}

/// Destroy a pollset (C-style API)
pub fn nextgcore_pollset_destroy(pollset: NextgcorePollset) {
    drop(pollset);
}
...
mod tests {
    ...
    fn test_pollset_create() {
        let pollset = NextgcorePollset::create(10);
        assert!(pollset.is_some());
    }

    #[test]
    fn test_pollset_notify() {
        let pollset = NextgcorePollset::create(10).unwrap();
    ...
    fn test_pollset_add_remove() {
        let mut pollset = NextgcorePollset::create(10).unwrap();
...
/// Async pollset manager wrapping NextgcorePollset
pub struct AsyncPollManager {
    pollset: Arc<Mutex<NextgcorePollset>>,
...
impl AsyncPollManager {
    ...
    pub fn new(capacity: usize) -> Option<Self> {
        let pollset = NextgcorePollset::create(capacity)?;
    ...
    pub fn spawn_poll_loop(&self, timeout: NextgcoreTime) {
...
impl AsyncPollHandle {
    ...
    pub fn fd(&self) -> NextgcoreSocket {
        self.fd
    }
...
mod async_tests {
    ...
    async fn test_async_poll_handle() {
        ...
        // Simulate event
        handle.mark_ready(NEXTGCORE_POLLIN);

        // Wait for readable
        let result = handle.readable().await;
        assert_eq!(result & NEXTGCORE_POLLIN, NEXTGCORE_POLLIN);
