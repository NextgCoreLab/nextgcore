...
//!
//! Exact port of lib/core/nextgcore-list.h

use std::ptr::NonNull;

/// List node - embedded in structures that participate in lists
#[repr(C)]
pub struct NextgcoreLnode {
    pub prev: Option<NonNull<NextgcoreLnode>>,
    pub next: Option<NonNull<NextgcoreLnode>>,
}
...
impl Default for NextgcoreLnode {
    fn default() -> Self {
        Self::new()
    }
}
...
#[repr(C)]
pub struct NextgcoreList {
    pub prev: Option<NonNull<NextgcoreLnode>>, // Points to last element
    pub next: Option<NonNull<NextgcoreLnode>>, // Points to first element
}
...
impl NextgcoreList {
    ...
    pub fn first(&self) -> Option<NonNull<NextgcoreLnode>> {
        self.next
    }

    pub fn last(&self) -> Option<NonNull<NextgcoreLnode>> {
        self.prev
    }
    ...
    /// Add node to end of list (identical to nextgcore_list_add)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid and not already in a list.
    pub unsafe fn add(&mut self, node: NonNull<NextgcoreLnode>) {
    ...
    /// Add node to beginning of list (identical to nextgcore_list_prepend)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid and not already in a list.
    pub unsafe fn prepend(&mut self, node: NonNull<NextgcoreLnode>) {
        let node_ptr = node.as_ptr();
        (*node_ptr).prev = None;
    ...
    /// Remove node from list (identical to nextgcore_list_remove)
    ///
    /// # Safety
    /// The caller must ensure the node is actually in this list.
    pub unsafe fn remove(&mut self, node: NonNull<NextgcoreLnode>) {
        let node_ptr = node.as_ptr();
    ...
    /// Insert node after another node (identical to nextgcore_list_insert_next)
    ///
    /// # Safety
    ...
    /// Insert node before another node (identical to nextgcore_list_insert_prev)
    ///
    /// # Safety
...
impl Default for NextgcoreList {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct NextgcoreListIter {
    current: Option<NonNull<NextgcoreLnode>>,
}

impl Iterator for NextgcoreListIter {
    type Item = NonNull<NextgcoreLnode>;
...
impl NextgcoreList {
    /// Copy list pointers from another list (identical to nextgcore_list_copy)
    pub fn copy_from(&mut self, src: &NextgcoreList) {
        self.prev = src.prev;
        self.next = src.next;
    }

    /// Check if a node exists in the list (identical to nextgcore_list_exists)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid.
    pub unsafe fn exists(&self, node: NonNull<NextgcoreLnode>) -> bool {
        let mut current = self.next;
    ...
    pub fn iter(&self) -> NextgcoreListIter {
        NextgcoreListIter { current: self.next }
    }

    /// Get the next node after the given node (identical to nextgcore_list_next)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid.
    pub unsafe fn next_node(node: NonNull<NextgcoreLnode>) -> Option<NonNull<NextgcoreLnode>> {
        (*node.as_ptr()).next
    }

    /// Get the previous node before the given node (identical to nextgcore_list_prev)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid.
    pub unsafe fn prev_node(node: NonNull<NextgcoreLnode>) -> Option<NonNull<NextgcoreLnode>> {
        (*node.as_ptr()).prev
    }

    /// Insert node in sorted order using a comparison function
    /// (identical to nextgcore_list_insert_sorted)
    ///
    /// # Safety
    /// The caller must ensure the node pointer is valid and not already in a list.
    pub unsafe fn insert_sorted<F>(&mut self, node: NonNull<NextgcoreLnode>, compare: F)
    where
        F: Fn(NonNull<NextgcoreLnode>, NonNull<NextgcoreLnode>) -> std::cmp::Ordering,
    {
        let mut current = self.next;
        while let Some(iter) = current {
...
mod tests {
    ...
    #[test]
    fn test_list_add() {
        let mut list = NextgcoreList::new();
    ...
    fn test_list_insert_prev() {
        ...
        unsafe {
            ...
            let second = NextgcoreList::next_node(first).unwrap();
            let third = NextgcoreList::next_node(second).unwrap();
    ...
    fn test_list_insert_sorted() {
        ...
        unsafe {
            ...
            let second = NextgcoreList::next_node(first).unwrap();
            let third = NextgcoreList::next_node(second).unwrap();
            let fourth = NextgcoreList::next_node(third).unwrap();
...
mod proptests {
    ...
    impl NodeStorage {
        ...
        fn add_node(&mut self, value: i32) -> NonNull<NextgcoreLnode> {
            let node = Box::new(TestNodeProp {
                lnode: NextgcoreLnode::new(),
                value,
            });
            let ptr = NonNull::new(&node.lnode as *const _ as *mut NextgcoreLnode).unwrap();
            self.nodes.push(node);
            ptr
        }

        fn get_node_ptr(&self, idx: usize) -> Option<NonNull<NextgcoreLnode>> {
            self.nodes
                .get(idx)
                .map(|n| NonNull::new(&n.lnode as *const _ as *mut NextgcoreLnode).unwrap())
        }
    }

    /// Extract values from list in order
    fn extract_list_values(list: &NextgcoreList, storage: &NodeStorage) -> Vec<i32> {
