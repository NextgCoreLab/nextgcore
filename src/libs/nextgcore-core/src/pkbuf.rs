...
//!
//! Exact port of lib/core/nextgcore-pkbuf.h and nextgcore-pkbuf.c
//!
...
/// Cluster - shared buffer storage (identical to nextgcore_cluster_t)
#[derive(Debug)]
struct NextgcoreCluster {
    buffer: Vec<u8>,
    size: usize,
}

impl NextgcoreCluster {
...
/// Packet buffer for network data (identical to nextgcore_pkbuf_t)
///
/// Memory layout:
/// ```text
...
/// head             data               tail             end
/// ```
pub struct NextgcorePkbuf {
    /// Shared cluster storage
    cluster: Arc<NextgcoreCluster>,
...
impl NextgcorePkbuf {
    /// Create a new packet buffer with given capacity (identical to nextgcore_pkbuf_alloc)
    pub fn new(size: usize) -> Self {
        let cluster_size = Self::select_cluster_size(size);
        let cluster = Arc::new(NextgcoreCluster::new(cluster_size));
    ...
    /// Get tailroom (identical to nextgcore_pkbuf_tailroom)
    #[inline]
    pub fn tailroom(&self) -> usize {
        self.end - self.tail
    }

    /// Get headroom (identical to nextgcore_pkbuf_headroom)
    #[inline]
    pub fn headroom(&self) -> usize {
        self.data - self.head
    }

    /// Reserve headroom (identical to nextgcore_pkbuf_reserve)
    ...
    /// Add space at tail and return mutable slice (identical to nextgcore_pkbuf_put)
    ...
    /// Put a u8 value at tail (identical to nextgcore_pkbuf_put_u8)
    #[inline]
    pub fn put_u8(&mut self, val: u8) {
        self.put(1)[0] = val;
    }

    /// Put a u16 value at tail in big-endian (identical to nextgcore_pkbuf_put_u16)
    #[inline]
    ...
    /// Put a u32 value at tail in big-endian (identical to nextgcore_pkbuf_put_u32)
    #[inline]
    ...
    /// Copy data to tail (identical to nextgcore_pkbuf_put_data)
    pub fn put_data(&mut self, data: &[u8]) {
        let dest = self.put(data.len());
        dest.copy_from_slice(data);
    }

    /// Add space at head and return mutable slice (identical to nextgcore_pkbuf_push)
    ...
    /// Remove data from head (identical to nextgcore_pkbuf_pull)
    ...
    pub fn pull_unchecked(&mut self, len: usize) -> &[u8] {
        self.pull(len).expect("value expected")
    }

    /// Trim buffer to specified length (identical to nextgcore_pkbuf_trim)
    ...
    /// Copy the packet buffer (identical to nextgcore_pkbuf_copy)
    pub fn copy(&self) -> Self {
        let mut new_pkbuf = NextgcorePkbuf::new(self.len);
...
impl Clone for NextgcorePkbuf {
    fn clone(&self) -> Self {
        self.copy()
    }
}

impl Default for NextgcorePkbuf {
    fn default() -> Self {
        Self::new(CLUSTER_2048) // Default MTU-sized buffer
    }
}

impl std::fmt::Debug for NextgcorePkbuf {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NextgcorePkbuf")
            .field("len", &self.len)
...
mod tests {
    ...
    #[test]
    fn test_pkbuf_new() {
        let pkbuf = NextgcorePkbuf::new(100);
    ...
    fn test_cluster_size_selection() {
        assert_eq!(NextgcorePkbuf::select_cluster_size(50), CLUSTER_128);
        assert_eq!(NextgcorePkbuf::select_cluster_size(128), CLUSTER_128);
        assert_eq!(NextgcorePkbuf::select_cluster_size(129), CLUSTER_256);
        assert_eq!(NextgcorePkbuf::select_cluster_size(1500), CLUSTER_2048);
        assert_eq!(NextgcorePkbuf::select_cluster_size(9000), CLUSTER_32768);
    }
    ...
    mod prop_tests {
        ...
        proptest! {
            ...
            fn prop_length_invariant(
                ...
                let mut pkbuf = NextgcorePkbuf::new(capacity);
            ...
            fn prop_put_data_retrievable(data in prop::collection::vec(any::<u8>(), 1..100)) {
                let mut pkbuf = NextgcorePkbuf::new(data.len() + 50);
                pkbuf.put_data(&data);
            ...
            fn prop_push_prepends(
                ...
                let mut pkbuf = NextgcorePkbuf::new(total_size + 50);
            ...
            fn prop_cluster_size_sufficient(size in 1..100000usize) {
                let cluster_size = NextgcorePkbuf::select_cluster_size(size);
