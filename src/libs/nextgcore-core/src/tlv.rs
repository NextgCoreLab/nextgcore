...
//!
//! Exact port of lib/core/nextgcore-tlv.h and nextgcore-tlv.c
//!
...
pub const NEXTGCORE_TLV_MODE_T1_L1: u8 = 1;
pub const NEXTGCORE_TLV_MODE_T1_L2: u8 = 2;
pub const NEXTGCORE_TLV_MODE_T1_L2_I1: u8 = 3;
pub const NEXTGCORE_TLV_MODE_T2_L2: u8 = 4;
pub const NEXTGCORE_TLV_MODE_T1: u8 = 5;
...
/// TLV element (identical to nextgcore_tlv_t)
#[derive(Debug, Clone)]
pub struct NextgcoreTlv {
    /// TLV mode
    pub mode: u8,
    /// Type field
    pub tlv_type: u32,
...
impl NextgcoreTlv {
    ...
    pub fn header_size(mode: u8) -> usize {
        match mode {
            NEXTGCORE_TLV_MODE_T1_L1 => 2,    // 1 + 1
            NEXTGCORE_TLV_MODE_T1_L2 => 3,    // 1 + 2
            NEXTGCORE_TLV_MODE_T1_L2_I1 => 4, // 1 + 2 + 1
            NEXTGCORE_TLV_MODE_T2_L2 => 4,    // 2 + 2
            NEXTGCORE_TLV_MODE_T1 => 1,       // 1 (no length)
            _ => 0,
        }
    }

    /// Encode TLV to bytes (identical to nextgcore_tlv_render)
    ...
    /// Decode TLV from bytes (identical to nextgcore_tlv_parse_block)
    ...
    /// Add embedded TLV
    pub fn add_embedded(&mut self, tlv: NextgcoreTlv) {
...
impl Default for NextgcoreTlv {
    fn default() -> Self {
        NextgcoreTlv {
            mode: NEXTGCORE_TLV_MODE_T2_L2,
            tlv_type: 0,
            length: 0,
            instance: 0,
            value: Vec::new(),
...
pub struct NextgcoreTlvMsg {
    /// TLV mode for this message
    pub mode: u8,
    /// TLV elements
    pub elements: Vec<NextgcoreTlv>,
}

impl NextgcoreTlvMsg {
    ...
    /// Decode TLV elements from buffer (identical to nextgcore_tlv_parse_block)
    pub fn decode(buf: &[u8], mode: u8) -> Result<Self, TlvError> {
        let mut msg = NextgcoreTlvMsg::new(mode);
        let mut offset = 0;

        while offset < buf.len() {
            let (tlv, len) = NextgcoreTlv::decode(&buf[offset..], mode)?;
    ...
    /// Find element by type (identical to nextgcore_tlv_find)
    pub fn find(&self, tlv_type: u32) -> Option<&NextgcoreTlv> {
        self.elements.iter().find(|e| e.tlv_type == tlv_type)
    }

    /// Find element by type and instance
    pub fn find_with_instance(&self, tlv_type: u32, instance: u8) -> Option<&NextgcoreTlv> {
        self.elements
            .iter()
            .find(|e| e.tlv_type == tlv_type && e.instance == instance)
    }

    /// Get total encoded size (identical to nextgcore_tlv_calc_length)
    pub fn encoded_size(&self) -> usize {
        self.elements.iter().map(|e| e.encoded_size()).sum()
    }

    /// Get element count (identical to nextgcore_tlv_calc_count)
...
mod tests {
    ...
    #[test]
    fn test_tlv_t1_l1() {
        let tlv = NextgcoreTlv::new(NEXTGCORE_TLV_MODE_T1_L1, 0x42, vec![1, 2, 3, 4]);
    ...
    #[test]
    fn test_tlv_t1_l2() {
        let tlv = NextgcoreTlv::new(NEXTGCORE_TLV_MODE_T1_L2, 0x42, vec![1, 2, 3, 4]);
        ...
        let (decoded, _) = NextgcoreTlv::decode(&buf[..len], NEXTGCORE_TLV_MODE_T1_L2).unwrap();
        assert_eq!(decoded.tlv_type, 0x42);
        assert_eq!(decoded.length, 4);
    }
    ...
    fn test_tlv_t1_l2_i1() {
        ...
        let (decoded, _) = NextgcoreTlv::decode(&buf[..len], NEXTGCORE_TLV_MODE_T1_L2_I1).unwrap();
        assert_eq!(decoded.tlv_type, 0x42);
    ...
    #[test]
    fn test_tlv_t2_l2() {
        let tlv = NextgcoreTlv::new(NEXTGCORE_TLV_MODE_T2_L2, 0x1234, vec![1, 2, 3, 4]);
        ...
        let (decoded, _) = NextgcoreTlv::decode(&buf[..len], NEXTGCORE_TLV_MODE_T2_L2).unwrap();
        assert_eq!(decoded.tlv_type, 0x1234);
    }

    #[test]
    fn test_tlv_msg() {
        let mut msg = NextgcoreTlvMsg::new(NEXTGCORE_TLV_MODE_T1_L1);
        ...
        let decoded = NextgcoreTlvMsg::decode(&buf[..len], NEXTGCORE_TLV_MODE_T1_L1).unwrap();
    ...
    fn test_tlv_value_helpers() {
        let tlv = NextgcoreTlv::new(NEXTGCORE_TLV_MODE_T1_L1, 0x01, vec![0x12, 0x34, 0x56, 0x78]);
    ...
    fn test_tlv_round_trip() {
        ...
        for mode in modes {
            let original = NextgcoreTlv::with_instance(mode, 0x42, 0x05, vec![1, 2, 3, 4, 5]);

            let mut buf = [0u8; 100];
            let len = original.encode(&mut buf).unwrap();

            let (decoded, decoded_len) = NextgcoreTlv::decode(&buf[..len], mode).unwrap();
            ...
            if mode == NEXTGCORE_TLV_MODE_T1_L2_I1 {
    ...
    fn test_tlv_unexpected_end() {
        ...
        let result = NextgcoreTlv::decode(&buf, NEXTGCORE_TLV_MODE_T1_L1);
...
mod proptests {
    ...
    fn tlv_mode_strategy() -> impl Strategy<Value = u8> {
        prop_oneof![
            Just(NEXTGCORE_TLV_MODE_T1_L1),
            Just(NEXTGCORE_TLV_MODE_T1_L2),
            Just(NEXTGCORE_TLV_MODE_T1_L2_I1),
            Just(NEXTGCORE_TLV_MODE_T2_L2),
        ]
    }
    ...
    proptest! {
        ...
        fn prop_tlv_round_trip(
            ...
            let actual_type = match mode {
                NEXTGCORE_TLV_MODE_T1_L1 | NEXTGCORE_TLV_MODE_T1_L2 | NEXTGCORE_TLV_MODE_T1_L2_I1 => tlv_type & 0xFF,
                NEXTGCORE_TLV_MODE_T2_L2 => tlv_type & 0xFFFF,
                _ => tlv_type,
            };

            // Constrain length based on mode
            let actual_len = match mode {
                NEXTGCORE_TLV_MODE_T1_L1 => value_len.min(255),
                _ => value_len,
            };
            ...
            let original = NextgcoreTlv::with_instance(mode, actual_type, instance, value.clone());

            let mut buf = vec![0u8; original.encoded_size() + 10];
            let encoded_len = original.encode(&mut buf).unwrap();

            let (decoded, decoded_len) = NextgcoreTlv::decode(&buf[..encoded_len], mode).unwrap();
        ...
        fn prop_encoded_size_accurate(
            ...
            let tlv = NextgcoreTlv::new(mode, 0x42, value);
        ...
        fn prop_header_size_correct(mode in tlv_mode_strategy()) {
            let expected = match mode {
                NEXTGCORE_TLV_MODE_T1_L1 => 2,
                NEXTGCORE_TLV_MODE_T1_L2 => 3,
                NEXTGCORE_TLV_MODE_T1_L2_I1 => 4,
                NEXTGCORE_TLV_MODE_T2_L2 => 4,
                NEXTGCORE_TLV_MODE_T1 => 1,
                _ => 0,
            };
            prop_assert_eq!(NextgcoreTlv::header_size(mode), expected);
        }
        ...
        fn prop_tlv_msg_round_trip(
            ...
            for i in 0..num_elements {
                let tlv_type = match mode {
                    NEXTGCORE_TLV_MODE_T2_L2 => (i as u32 + 1) * 0x100,
                    _ => (i as u32 + 1) & 0xFF,
                };
            ...
            let decoded = NextgcoreTlvMsg::decode(&buf[..encoded_len], mode).unwrap();
        ...
        fn prop_find_returns_correct_element(
            ...
            let target_type = match mode {
                NEXTGCORE_TLV_MODE_T2_L2 => (actual_idx as u32 + 1) * 0x100,
        ...
        fn prop_value_helpers_correct(
            ...
            let tlv = NextgcoreTlv::new(NEXTGCORE_TLV_MODE_T1_L1, 0x01, vec![b0, b1, b2, b3]);
        ...
        fn prop_empty_value_round_trip(mode in tlv_mode_strategy()) {
            let tlv = NextgcoreTlv::new(mode, 0x42, vec![]);
