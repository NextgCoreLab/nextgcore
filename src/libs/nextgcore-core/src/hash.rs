...
//!
//! Exact port of lib/core/nextgcore-hash.h and nextgcore-hash.c
//!
...
pub const NEXTGCORE_HASH_KEY_STRING: i32 = -1;

/// Initial maximum size (tunable == 2^n - 1)
const INITIAL_MAX: usize = 15;

/// Hash entry in the chain
struct NextgcoreHashEntry {
    next: Option<Box<NextgcoreHashEntry>>,
    hash: u32,
    key: Vec<u8>,
    klen: i32,
    val: *mut (),
...
pub struct NextgcoreHash {
    array: Vec<Option<Box<NextgcoreHashEntry>>>,
    count: usize,
    max: usize,
    seed: u32,
...
impl NextgcoreHash {
    /// Create a new hash table (identical to nextgcore_hash_make)
    pub fn new() -> Self {
    ...
    /// Create a hash table with a custom hash function (identical to nextgcore_hash_make_custom)
    ...
    fn hash_default(&self, key: &[u8], klen: &mut i32) -> u32 {
        ...
        if *klen == NEXTGCORE_HASH_KEY_STRING {
            let mut len = 0i32;
            for &byte in key {
                if byte == 0 {
                    break;
    ...
    fn expand_array(&mut self) {
        ...
        let mut new_array: Vec<Option<Box<NextgcoreHashEntry>>> = Vec::with_capacity(new_max + 1);
        for _ in 0..=new_max {
            new_array.push(None);
        }
    ...
    /// Set a key-value pair (identical to nextgcore_hash_set)
    pub fn set(&mut self, key: &[u8], klen: i32, val: Option<*mut ()>) {
        ...
        if !found {
            if let Some(val) = val {
                ...
                if !exists {
                    let new_entry = Box::new(NextgcoreHashEntry {
    ...
    fn delete_entry(&mut self, index: usize, hash: u32, klen: i32, key: &[u8]) {
        ...
        if let Some(ref entry) = slot {
            if entry.hash == hash && entry.klen == klen && entry.key == key {
                let mut removed = slot.take().expect("slot was Some");
        ...
        while let Some(entry) = current {
            if let Some(ref next_entry) = entry.next {
                if next_entry.hash == hash && next_entry.klen == klen && next_entry.key == key {
                    let mut removed = entry.next.take().expect("next was Some");
    ...
    /// Get a value by key (identical to nextgcore_hash_get)
    ...
    /// Get or set a value (identical to nextgcore_hash_get_or_set)
    ...
    /// Get number of entries (identical to nextgcore_hash_count)
    pub fn count(&self) -> usize {
        self.count
    }

    /// Clear all entries (identical to nextgcore_hash_clear)
    ...
    pub fn iter(&self) -> NextgcoreHashIter<'_> {
        NextgcoreHashIter {
            ht: self,
            index: 0,
            current: None,
...
impl Default for NextgcoreHash {
    fn default() -> Self {
        Self::new()
    }
}
...
pub struct NextgcoreHashIter<'a> {
    ht: &'a NextgcoreHash,
    index: usize,
    current: Option<&'a NextgcoreHashEntry>,
}

impl<'a> Iterator for NextgcoreHashIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(entry) = self.current {
            if let Some(ref next) = entry.next {
                ...
                let curr = self.current.expect("current was set");
...
/// The default hash function exposed for external use (identical to nextgcore_hashfunc_default)
pub fn nextgcore_hashfunc_default(key: &[u8], klen: &mut i32) -> u32 {
    let mut hash = 0u32;
...
pub struct NextgcoreHashMap<K, V> {
    map: HashMap<K, V>,
}

impl<K: Eq + Hash, V> NextgcoreHashMap<K, V> {
    pub fn new() -> Self {
        NextgcoreHashMap {
            map: HashMap::new(),
        }
...
impl<K: Eq + Hash, V> Default for NextgcoreHashMap<K, V> {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    mod prop_tests {
        ...
        proptest! {
            ...
            fn prop_hash_deterministic(key in prop::collection::vec(any::<u8>(), 1..64)) {
                ...
                let hash1 = nextgcore_hashfunc_default(&key, &mut klen1);
                let hash2 = nextgcore_hashfunc_default(&key, &mut klen2);
    ...
    fn test_hash_string_key() {
        ...
        ht.set(key, NEXTGCORE_HASH_KEY_STRING, Some(val_ptr));

        let result = ht.get(b"hello\0", NEXTGCORE_HASH_KEY_STRING);
        assert!(result.is_some());
    }
    ...
    fn test_hashfunc_default() {
        ...
        let hash1 = nextgcore_hashfunc_default(key, &mut klen);
