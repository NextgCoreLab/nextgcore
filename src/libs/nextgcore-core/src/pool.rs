...
//!
//! Exact port of lib/core/nextgcore-pool.h
//!
...
/// Pool ID type (identical to nextgcore_pool_id_t)
pub type NextgcorePoolId = i32;

/// Invalid pool ID constant
pub const NEXTGCORE_INVALID_POOL_ID: NextgcorePoolId = 0;
/// Minimum valid pool ID
pub const NEXTGCORE_MIN_POOL_ID: NextgcorePoolId = 1;
/// Maximum valid pool ID
pub const NEXTGCORE_MAX_POOL_ID: NextgcorePoolId = 0x7fffffff;

/// Trait for items that can be stored in a pool with ID
pub trait PoolItem {
    fn id(&self) -> NextgcorePoolId;
    fn set_id(&mut self, id: NextgcorePoolId);
}
...
impl<T> NextgcorePool<T> {
    /// Create a new pool with the given capacity (identical to nextgcore_pool_init/nextgcore_pool_create)
    ...
    /// Allocate an item from the pool (identical to nextgcore_pool_alloc)
    ...
    /// Free an item back to the pool (identical to nextgcore_pool_free)
    ...
    /// Get item by 1-based index (identical to nextgcore_pool_find)
    ...
    /// Get the 1-based index of an item (identical to nextgcore_pool_index)
    ...
    /// Get number of available slots (identical to nextgcore_pool_avail)
    pub fn available(&self) -> usize {
        self.avail
    }

    /// Get total capacity (identical to nextgcore_pool_size)
...
/// This matches the nextgcore_pool_id_calloc/nextgcore_pool_id_free functionality
pub struct NextgcorePoolWithId<T: PoolItem> {
    pool: NextgcorePool<T>,
    id_hash: HashMap<NextgcorePoolId, usize>,
    next_id: NextgcorePoolId,
}

impl<T: PoolItem + Default> NextgcorePoolWithId<T> {
    /// Create a new pool with ID support
    pub fn new(name: &str, size: usize) -> Self {
        NextgcorePoolWithId {
            pool: NextgcorePool::new(name, size),
            id_hash: HashMap::new(),
            next_id: NEXTGCORE_MIN_POOL_ID,
        }
    }

    /// Allocate an item and assign it a unique ID (identical to nextgcore_pool_id_calloc)
    pub fn alloc(&mut self) -> Option<(NextgcorePoolId, usize)> {
        let index = self.pool.alloc()?;

        // Generate next ID (wrapping from MIN to MAX)
        let id = self.next_id;
        self.next_id = if self.next_id == NEXTGCORE_MAX_POOL_ID {
    ...
    /// Free an item by its ID (identical to nextgcore_pool_id_free)
    pub fn free_by_id(&mut self, id: NextgcorePoolId) {
    ...
    /// Find item by ID (identical to nextgcore_pool_find_by_id)
    pub fn find_by_id(&self, id: NextgcorePoolId) -> Option<&T> {
        let index = self.id_hash.get(&id)?;
        self.pool.find(*index)
    }

    /// Find mutable item by ID
    pub fn find_by_id_mut(&mut self, id: NextgcorePoolId) -> Option<&mut T> {
        let index = *self.id_hash.get(&id)?;
...
/// Helper macro to generate next ID with wrapping (identical to NEXTGCORE_NEXT_ID)
#[macro_export]
macro_rules! nextgcore_next_id {
...
mod tests {
    ...
    #[test]
    fn test_pool_new() {
        let pool: NextgcorePool<TestItem> = NextgcorePool::new("test_pool", 10);
    ...
    fn test_pool_alloc_free() {
        let mut pool: NextgcorePool<TestItem> = NextgcorePool::new("test_pool", 5);
    ...
    #[test]
    fn test_pool_full() {
        let mut pool: NextgcorePool<TestItem> = NextgcorePool::new("test_pool", 3);
    ...
    fn test_pool_with_id() {
        let mut pool: NextgcorePoolWithId<TestItem> = NextgcorePoolWithId::new("test_pool", 5);

        // Allocate with ID
        let (id1, _idx1) = pool.alloc().unwrap();
        let (id2, _idx2) = pool.alloc().unwrap();

        assert!((NEXTGCORE_MIN_POOL_ID..=NEXTGCORE_MAX_POOL_ID).contains(&id1));
        assert!((NEXTGCORE_MIN_POOL_ID..=NEXTGCORE_MAX_POOL_ID).contains(&id2));
        assert_ne!(id1, id2);
    ...
    mod prop_tests {
        ...
        fn pool_op_strategy() -> impl Strategy<Value = PoolOp> {
            prop_oneof![Just(PoolOp::Alloc), (0..10usize).prop_map(PoolOp::Free),]
        }
        ...
        proptest! {
            ...
            fn prop_capacity_invariant(
                ...
                let mut pool: NextgcorePool<TestItem> = NextgcorePool::new("test", size);
            ...
            /// Property 8: ID uniqueness in NextgcorePoolWithId
            /// All allocated IDs should be unique
            #[test]
            fn prop_id_uniqueness(size in 1..30usize) {
                let mut pool: NextgcorePoolWithId<TestItem> = NextgcorePoolWithId::new("test", size);
                let mut ids: HashSet<NextgcorePoolId> = HashSet::new();
            ...
            fn prop_id_lookup(size in 1..20usize) {
                let mut pool: NextgcorePoolWithId<TestItem> = NextgcorePoolWithId::new("test", size);
                let mut id_to_idx: HashMap<NextgcorePoolId, usize> = HashMap::new();
