...
//!
//! Exact port of lib/core/nextgcore-timer.h and nextgcore-timer.c
//!
...
pub const NEXTGCORE_NO_WAIT_TIME: Duration = Duration::ZERO;
pub const NEXTGCORE_INFINITE_TIME: Duration = Duration::MAX;

/// Timer ID type
pub type NextgcoreTimerId = u64;

/// Timer callback function type
pub type NextgcoreTimerCallback<T> = fn(&mut T);

/// Timer structure (identical to nextgcore_timer_t)
#[derive(Debug)]
pub struct NextgcoreTimer<T> {
    /// Timer ID
    id: NextgcoreTimerId,
    /// Callback function
...
impl<T> NextgcoreTimer<T> {
    ...
    /// Get timer ID
    pub fn id(&self) -> NextgcoreTimerId {
        self.id
    }
...
/// Timer manager for handling multiple timers (identical to nextgcore_timer_mgr_t)
pub struct NextgcoreTimerMgr<T> {
    /// Timer storage
    timers: Vec<Option<NextgcoreTimer<T>>>,
    /// Free list of timer indices
    free_indices: Vec<usize>,
    /// Tree for ordering timers by timeout (maps timeout -> timer index)
    tree: BTreeMap<TimerKey, usize>,
    /// Next timer ID
    next_id: NextgcoreTimerId,
    /// Capacity
    capacity: usize,
...
impl<T> NextgcoreTimerMgr<T> {
    /// Create a new timer manager with given capacity (identical to nextgcore_timer_mgr_create)
    ...
    pub fn add(&mut self, callback: NextgcoreTimerCallback<T>, data: T) -> Option<NextgcoreTimerId> {
        ...
        let timer = NextgcoreTimer::new(id, Some(callback), Some(data));
    ...
    pub fn add_simple(&mut self) -> Option<NextgcoreTimerId> {
        let index = self.free_indices.pop()?;

        let id = self.next_id;
        self.next_id += 1;

        let timer: NextgcoreTimer<T> = NextgcoreTimer::new(id, None, None);
        self.timers[index] = Some(timer);

        Some(id)
    }

    /// Delete a timer (identical to nextgcore_timer_delete)
    pub fn delete(&mut self, id: NextgcoreTimerId) {
    ...
    /// Start a timer (identical to nextgcore_timer_start)
    pub fn start(&mut self, id: NextgcoreTimerId, duration: Duration) {
        if let Some(index) = self.find_index(id) {
    ...
    /// Stop a timer (identical to nextgcore_timer_stop)
    pub fn stop(&mut self, id: NextgcoreTimerId) {
        if let Some(index) = self.find_index(id) {
            // Get timer info for tree operations
    ...
    /// Get time until next timer expires (identical to nextgcore_timer_mgr_next)
    pub fn next(&self) -> Duration {
        if let Some((key, _)) = self.tree.first_key_value() {
            ...
            if key.timeout > now_nanos {
                ...
                NEXTGCORE_NO_WAIT_TIME
            }
        } else {
            NEXTGCORE_INFINITE_TIME
        }
    }

    /// Expire all timers that have timed out (identical to nextgcore_timer_mgr_expire)
    /// Returns list of expired timer IDs
    pub fn expire(&mut self) -> Vec<NextgcoreTimerId> {
        let now_nanos = self.instant_to_nanos(Instant::now());
    ...
    /// Get timer by ID
    pub fn get(&self, id: NextgcoreTimerId) -> Option<&NextgcoreTimer<T>> {
        self.find_index(id)
            .and_then(|idx| self.timers[idx].as_ref())
    }

    /// Get mutable timer by ID
    pub fn get_mut(&mut self, id: NextgcoreTimerId) -> Option<&mut NextgcoreTimer<T>> {
        self.find_index(id)
    ...
    fn find_index(&self, id: NextgcoreTimerId) -> Option<usize> {
        self.timers
            .iter()
...
impl<T> Default for NextgcoreTimerMgr<T> {
    fn default() -> Self {
        Self::new(1024)
    }
}
...
impl DistributedTimerCoordinator {
    ...
    pub fn sync_timer(&self, timer_id: NextgcoreTimerId, local_expiry_nanos: u128) -> u128 {
        match self.mode {
    ...
    pub fn get_sync_expiry(&self, timer_id: NextgcoreTimerId) -> Option<u128> {
    ...
    pub fn unsync_timer(&self, timer_id: NextgcoreTimerId) {
        let mut records = self.sync_records.lock().unwrap();
...
mod tests {
    ...
    fn test_timer_mgr_new() {
        let mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(10);
    ...
    fn test_timer_add_delete() {
        let mut mgr: NextgcoreTimerMgr<i32> = NextgcoreTimerMgr::new(10);
    ...
    fn test_timer_start_stop() {
        let mut mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(10);
    ...
    fn test_timer_next() {
        ...
        assert_eq!(mgr.next(), NEXTGCORE_INFINITE_TIME);
    ...
    mod prop_tests {
        ...
        proptest! {
            ...
            fn prop_capacity_invariant(
                ...
                let mut mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(capacity);
            ...
            fn prop_unique_ids(count in 1..20usize) {
                let mut mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(count + 10);
                let mut ids: HashSet<NextgcoreTimerId> = HashSet::new();
            ...
            fn prop_start_stop_count(timer_count in 1..10usize) {
                let mut mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(timer_count + 5);
                let mut ids = Vec::new();
            ...
            fn prop_next_infinite_when_empty(capacity in 5..20usize) {
                let mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(capacity);
                prop_assert_eq!(mgr.next(), NEXTGCORE_INFINITE_TIME, "next should be INFINITE when empty");
            }
            ...
            fn prop_expire_only_timed_out(
                ...
                let mut mgr: NextgcoreTimerMgr<()> = NextgcoreTimerMgr::new(20);
    ...
    fn test_get_all_synced_timers() {
        ...
        let timer_ids: Vec<NextgcoreTimerId> = all_timers.iter().map(|r| r.timer_id).collect();
