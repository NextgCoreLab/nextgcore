//! Thread utilities
//!
//! Exact port of lib/core/nextgcore-thread.h and nextgcore-thread.c

use std::sync::{Arc, Condvar, Mutex};
use std::thread::{self, JoinHandle};
use std::time::Duration;

use crate::errno::{NEXTGCORE_ERROR, NEXTGCORE_OK, NEXTGCORE_TIMEUP};
use crate::time::{nextgcore_get_monotonic_time, NextgcoreTime};

/// Mutex type (wrapper around std::sync::Mutex)
pub type NextgcoreThreadMutex<T> = Mutex<T>;

/// Condition variable type (wrapper around std::sync::Condvar)
pub type NextgcoreThreadCond = Condvar;

/// Thread ID type
pub type NextgcoreThreadId = JoinHandle<()>;

/// Initialize a mutex (no-op in Rust, mutex is initialized on creation)
#[inline]
pub fn nextgcore_thread_mutex_init<T>(data: T) -> NextgcoreThreadMutex<T> {
    Mutex::new(data)
}
...
#[inline]
pub fn nextgcore_thread_cond_init() -> NextgcoreThreadCond {
    Condvar::new()
}

/// Wait on a condition variable
#[inline]
pub fn nextgcore_thread_cond_wait<'a, T>(
    cond: &NextgcoreThreadCond,
...
/// Returns NEXTGCORE_OK on success, NEXTGCORE_TIMEUP on timeout, NEXTGCORE_ERROR on error
#[inline]
pub fn nextgcore_thread_cond_timedwait<'a, T>(
    cond: &NextgcoreThreadCond,
    guard: std::sync::MutexGuard<'a, T>,
    timeout: NextgcoreTime,
    ...
    match result {
        Ok((guard, timeout_result)) => {
            if timeout_result.timed_out() {
                (guard, NEXTGCORE_TIMEUP)
            } else {
                (guard, NEXTGCORE_OK)
            }
        }
        Err(e) => (e.into_inner().0, NEXTGCORE_ERROR),
    }
}

/// Signal a condition variable (wake one waiter)
#[inline]
pub fn nextgcore_thread_cond_signal(cond: &NextgcoreThreadCond) {
    cond.notify_one();
}

/// Broadcast to a condition variable (wake all waiters)
#[inline]
pub fn nextgcore_thread_cond_broadcast(cond: &NextgcoreThreadCond) {
    cond.notify_all();
}
...
/// Thread structure (identical to nextgcore_thread_t)
pub struct NextgcoreThread {
    handle: Option<JoinHandle<()>>,
    state: Arc<(Mutex<ThreadState>, Condvar)>,
}

impl NextgcoreThread {
    /// Create a new thread (identical to nextgcore_thread_create)
    ...
    /// Destroy the thread (identical to nextgcore_thread_destroy)
    /// Waits up to 5 seconds for thread to finish
    pub fn destroy(mut self) {
        let deadline = nextgcore_get_monotonic_time() + 5 * 1000 * 1000; // 5 seconds

        // Wait for thread to finish (up to 5 seconds)
        while nextgcore_get_monotonic_time() <= deadline {
            {
                let guard = self.state.0.lock().unwrap_or_else(|e| e.into_inner());
                if !guard.running {
                    break;
...
pub fn nextgcore_thread_create<F>(func: F) -> Option<NextgcoreThread>
where
    F: FnOnce() + Send + 'static,
{
    NextgcoreThread::create(func)
}

/// Destroy a thread (C-style API)
pub fn nextgcore_thread_destroy(thread: NextgcoreThread) {
    thread.destroy();
}
...
mod tests {
    ...
    fn test_thread_create_destroy() {
        ...
        let thread = NextgcoreThread::create(move || {
    ...
    #[test]
    fn test_mutex_lock() {
        let mutex = nextgcore_thread_mutex_init(42);
        {
            let guard = nextgcore_thread_mutex_lock(&mutex);
            assert_eq!(*guard, 42);
        }
    }
