...
//!
//! Exact port of lib/core/nextgcore-fsm.h and nextgcore-fsm.c
//!
...
//! [`NextgcoreFsm`].  The trait uses enum-based states which are idiomatic Rust and
//! avoid the unreliable function-pointer comparison required by [`NextgcoreFsm::check`].
//!
//! ## Quick start
//!
//! ```rust,ignore
//! use nextgcore_core::fsm::{StateMachine, FsmResult};
//!
...
///
/// Prefer this over the legacy [`NextgcoreFsm`] function-pointer approach.
...
/// FSM signal types (identical to nextgcore_fsm_signal_e)
...
pub type NextgcoreFsmHandler<S, E> = fn(&mut S, &mut E);

/// Finite State Machine structure (identical to nextgcore_fsm_t)
///
/// # Deprecation
///
/// This type uses function-pointer comparison in [`NextgcoreFsm::check`], which
...
#[repr(C)]
pub struct NextgcoreFsm<S, E> {
    /// Initial state handler
    pub init: Option<NextgcoreFsmHandler<S, E>>,
    /// Final state handler
    pub fini: Option<NextgcoreFsmHandler<S, E>>,
    /// Current state handler
    pub state: Option<NextgcoreFsmHandler<S, E>>,
    _phantom: PhantomData<(S, E)>,
}

#[allow(deprecated)]
impl<S, E> NextgcoreFsm<S, E> {
    /// Create a new FSM
    pub fn new() -> Self {
        NextgcoreFsm {
            init: None,
    ...
    pub fn with_handlers(init: NextgcoreFsmHandler<S, E>, fini: NextgcoreFsmHandler<S, E>) -> Self {
        NextgcoreFsm {
            init: Some(init),
            fini: Some(fini),
            state: None,
    ...
    /// Initialize FSM (identical to nextgcore_fsm_init)
    ...
    /// Transition to new state (identical to NEXTGCORE_FSM_TRAN macro)
    /// This sets the new state without calling entry/exit handlers
    #[inline]
    pub fn tran(&mut self, target: NextgcoreFsmHandler<S, E>) {
        self.state = Some(target);
    }

    /// Dispatch event to current state (identical to nextgcore_fsm_dispatch)
    ...
    /// Finalize FSM (identical to nextgcore_fsm_fini)
    ...
    // NOTE: The legacy `check()` method (NEXTGCORE_FSM_CHECK macro) was removed — it
    ...
    /// Get current state (identical to NEXTGCORE_FSM_STATE macro)
    #[inline]
    pub fn get_state(&self) -> Option<NextgcoreFsmHandler<S, E>> {
        self.state
    }

    /// Set state directly
    #[inline]
    pub fn set_state(&mut self, state: NextgcoreFsmHandler<S, E>) {
...
#[allow(deprecated)]
impl<S, E> Default for NextgcoreFsm<S, E> {
    fn default() -> Self {
        Self::new()
    }
}
...
#[allow(deprecated)]
impl<S, E> Clone for NextgcoreFsm<S, E> {
    fn clone(&self) -> Self {
        NextgcoreFsm {
            init: self.init,
            fini: self.fini,
...
#[allow(deprecated)]
impl<S, E> std::fmt::Debug for NextgcoreFsm<S, E> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NextgcoreFsm")
...
mod tests {
    ...
    #[test]
    fn test_fsm_new() {
        let fsm: NextgcoreFsm<TestContext, TestEvent> = NextgcoreFsm::new();
    ...
    fn test_fsm_init() {
        ...
        let mut event = TestEvent {
            signal: NEXTGCORE_FSM_ENTRY_SIG,
            data: 42,
        };
    ...
    #[test]
    fn test_fsm_tran() {
        let mut fsm: NextgcoreFsm<TestContext, TestEvent> = NextgcoreFsm::new();
    ...
    #[test]
    fn test_fsm_clone() {
        let mut fsm1: NextgcoreFsm<TestContext, TestEvent> =
    ...
    mod prop_tests {
        ...
        proptest! {
            ...
            fn prop_tran_sets_state(handler_idx in 0..3usize) {
                let mut fsm: NextgcoreFsm<i32, i32> = NextgcoreFsm::new();

                let handlers = [handler_1 as NextgcoreFsmHandler<i32, i32>, handler_2, handler_3];
            ...
            fn prop_init_calls_handler(_dummy in 0..1i32) {
                let mut fsm: NextgcoreFsm<i32, i32> = NextgcoreFsm::with_handlers(init_handler, fini_handler);
                let mut ctx = 999i32;
                let mut event = 0i32;
            ...
            fn prop_clone_preserves_state(handler_idx in 0..3usize) {
                let mut fsm1: NextgcoreFsm<i32, i32> = NextgcoreFsm::with_handlers(init_handler, fini_handler);
                let handlers = [handler_1 as NextgcoreFsmHandler<i32, i32>, handler_2, handler_3];
