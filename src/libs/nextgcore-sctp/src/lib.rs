...
pub const NEXTGCORE_S1AP_SCTP_PORT: u16 = 36412;

/// SGsAP SCTP port (3GPP TS 29.118)
pub const NEXTGCORE_SGSAP_SCTP_PORT: u16 = 29118;

/// NGAP SCTP port (3GPP TS 38.412)
pub const NEXTGCORE_NGAP_SCTP_PORT: u16 = 38412;

/// S1AP PPID (Payload Protocol Identifier)
pub const NEXTGCORE_SCTP_S1AP_PPID: u32 = 18;

/// X2AP PPID
pub const NEXTGCORE_SCTP_X2AP_PPID: u32 = 27;

/// SGsAP PPID
pub const NEXTGCORE_SCTP_SGSAP_PPID: u32 = 0;

/// NGAP PPID
pub const NEXTGCORE_SCTP_NGAP_PPID: u32 = 60;

/// NGAP PPID (alias for server module compatibility)
pub const NGAP_PPID: u32 = 60;

/// Maximum SDU length
pub const NEXTGCORE_MAX_SDU_LEN: usize = 8192;
    ...
    impl SctpAssociation {
        ...
        pub async fn connect(remote_addr: SocketAddr, config: SctpConfig) -> Result<Self> {
            let local_addr: SocketAddr = if remote_addr.is_ipv6() {
                "[::]:0".parse().expect("value expected")
            } else {
                "0.0.0.0:0".parse().expect("value expected")
            };
...
impl Default for NextgcoreSctpSock {
    fn default() -> Self {
...
#[inline]
pub fn nextgcore_sctp_ppid_in_pkbuf(pkbuf: &NextgcorePkbuf) -> u32 {
    pkbuf.param[0] as u32
}

/// Get stream number from packet buffer param[1]
#[inline]
pub fn nextgcore_sctp_stream_no_in_pkbuf(pkbuf: &NextgcorePkbuf) -> u16 {
    pkbuf.param[1] as u16
}

/// Set PPID in packet buffer param[0]
#[inline]
pub fn nextgcore_sctp_set_ppid_in_pkbuf(pkbuf: &mut NextgcorePkbuf, ppid: u32) {
    pkbuf.param[0] = ppid as u64;
}

/// Set stream number in packet buffer param[1]
#[inline]
pub fn nextgcore_sctp_set_stream_no_in_pkbuf(pkbuf: &mut NextgcorePkbuf, stream_no: u16) {
...
pub fn nextgcore_sctp_write_to_buffer(sctp: &mut NextgcoreSctpSock, pkbuf: NextgcorePkbuf) {
    sctp.write_queue.push_back(pkbuf);
}

/// Flush write queue and destroy socket
pub fn nextgcore_sctp_flush_and_destroy(sctp: &mut NextgcoreSctpSock) {
...
mod tests {
    ...
    #[test]
    fn test_constants() {
        assert_eq!(NEXTGCORE_S1AP_SCTP_PORT, 36412);
        assert_eq!(NEXTGCORE_NGAP_SCTP_PORT, 38412);
        assert_eq!(NEXTGCORE_SGSAP_SCTP_PORT, 29118);
        assert_eq!(NEXTGCORE_SCTP_S1AP_PPID, 18);
        assert_eq!(NEXTGCORE_SCTP_NGAP_PPID, 60);
        assert_eq!(NEXTGCORE_SCTP_X2AP_PPID, 27);
        assert_eq!(NEXTGCORE_SCTP_SGSAP_PPID, 0);
    ...
    fn test_default_constants() {
        ...
        assert_eq!(NEXTGCORE_MAX_SDU_LEN, 8192);
    }
    ...
    // ============================================================================
    // NextgcoreSctpInfo Tests
    // ============================================================================

    #[test]
    fn test_sctp_info_default() {
        let info = NextgcoreSctpInfo::default();
    ...
    // ============================================================================
    // NextgcoreSctpSock Tests
    // ============================================================================

    #[test]
    fn test_sctp_sock_default() {
        let sock = NextgcoreSctpSock::default();
    ...
    fn test_sctp_sock_new_stream() {
        let sock = NextgcoreSctpSock::new(SOCK_STREAM);
        assert_eq!(sock.sock_type, SOCK_STREAM);
    }

    #[test]
    fn test_sctp_sock_new_seqpacket() {
        let sock = NextgcoreSctpSock::new(SOCK_SEQPACKET);
    ...
    fn test_pkbuf_stream_zero() {
        let mut pkbuf = NextgcorePkbuf::new(100);
        nextgcore_sctp_set_stream_no_in_pkbuf(&mut pkbuf, 0);
        assert_eq!(nextgcore_sctp_stream_no_in_pkbuf(&pkbuf), 0);
    }

    #[test]
    fn test_pkbuf_stream_max() {
        let mut pkbuf = NextgcorePkbuf::new(100);
        nextgcore_sctp_set_stream_no_in_pkbuf(&mut pkbuf, u16::MAX);
        assert_eq!(nextgcore_sctp_stream_no_in_pkbuf(&pkbuf), u16::MAX);
    }
    ...
    #[test]
    fn test_init_final() {
        nextgcore_sctp_init(0);
        nextgcore_sctp_final();
    }

    #[test]
    fn test_init_with_port() {
        nextgcore_sctp_init(38412);
        nextgcore_sctp_final();
    }
    ...
    #[test]
    fn test_write_queue() {
        let mut sctp_sock = NextgcoreSctpSock::new(SOCK_STREAM);

        let mut pkbuf1 = NextgcorePkbuf::new(100);
        pkbuf1.put_data(&[1, 2, 3, 4]);

        let mut pkbuf2 = NextgcorePkbuf::new(100);
        pkbuf2.put_data(&[5, 6, 7, 8]);

        nextgcore_sctp_write_to_buffer(&mut sctp_sock, pkbuf1);
        nextgcore_sctp_write_to_buffer(&mut sctp_sock, pkbuf2);

        assert_eq!(sctp_sock.write_queue.len(), 2);

        nextgcore_sctp_flush_and_destroy(&mut sctp_sock);
    ...
    fn test_write_queue_single_packet() {
        ...
        nextgcore_sctp_write_to_buffer(&mut sctp_sock, pkbuf);
