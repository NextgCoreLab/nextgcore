...
fn make_u31(a: u8, b: u32, c: u8) -> u32 {
    ((a as u32) << 23) | ((b & 0x7FFF) << 8) | (c as u32)
}
...
impl ZucState {
    ...
    pub fn initialize(&mut self, k: &[u8; 16], iv: &[u8; 16]) {
        for i in 0..16 {
            self.lfsr[i] = make_u31(k[i], EK_D[i], iv[i]);
        }
        self.r1 = 0;
        self.r2 = 0;
        for _ in 0..32 {
...
pub fn zuc_eea3(
    ...
    for i in 0..l8 {
        c[i] = m[i] ^ ((z[i / 4] >> ((3 - i % 4) * 8)) & 0xff) as u8;
    }
    if lastbits > 0 && l8 > 0 {
        c[l8 - 1] &= 0x100u16.wrapping_sub(1 << lastbits) as u8;
    }
}
...
fn get_word(data: &[u32], i: u32) -> u32 {
    ...
    if ti == 0 {
        ...
        (data[idx] << ti) | (data.get(idx + 1).copied().unwrap_or(0) >> (32 - ti))
    }
}
fn get_bit(data: &[u8], i: u32) -> u8 {
    let idx = (i / 8) as usize;
    if idx < data.len() && (data[idx] & (1 << (7 - (i % 8)))) != 0 {
        1
    } else {
        0
    }
...
mod tests {
    ...
    fn test_zuc_eea3_3gpp() {
        ...
        let expected: [u8; 28] = [
            ...
            0x84, 0x94, 0x0e, 0xe1, 0xe4, 0xb0, 0x30, 0x23, 0x8c, 0xc8, 0x10, 0x00, 0x00, 0x00,
        ];
