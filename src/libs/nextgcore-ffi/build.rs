...
fn main() {
    ...
    let generate_bindings = env::var("NEXTGCORE_FFI_GENERATE_BINDINGS").is_ok();

    if !generate_bindings {
        // Use stub bindings for now - actual bindings will be generated
        // when NEXTGCORE_FFI_GENERATE_BINDINGS=1 is set
        println!("cargo:warning=Using stub FFI bindings. Set NEXTGCORE_FFI_GENERATE_BINDINGS=1 to generate from C headers.");
        return;
    }
...
"#;
    ...
    let bindings = bindgen::Builder::default()
        ...
        // Parse all nextgcore_ prefixed functions and types
        .allowlist_function("nextgcore_.*")
        .allowlist_type("nextgcore_.*")
        .allowlist_var("NEXTGCORE_.*")
        // Generate Rust enums for C enums
        .rustified_enum("nextgcore_.*")
        // Use core types
        .use_core()
