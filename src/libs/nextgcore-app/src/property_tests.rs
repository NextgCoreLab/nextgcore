...
mod tests {
    ...
    fn arb_max_conf() -> impl Strategy<Value = MaxConf> {
        (1u64..10000, 1u64..1000, 0u64..1000).prop_map(|(ue, peer, gtp_peer)| MaxConf {
            ue,
            peer,
            gtp_peer,
        })
    ...
    fn arb_global_conf() -> impl Strategy<Value = NextgcoreGlobalConf> {
        (
            arb_parameter_conf(),
            arb_max_conf(),
            arb_sockopt_conf(),
    ...
    fn arb_plmn_id() -> impl Strategy<Value = NextgcorePlmnId> {
        (100u16..999, 0u16..999, 2u8..=3)
            .prop_map(|(mcc, mnc, mnc_len)| NextgcorePlmnId::build(mcc, mnc, mnc_len))
    }

    // Strategy for generating valid local configurations
    fn arb_local_conf() -> impl Strategy<Value = NextgcoreLocalConf> {
        (
            ...
            .prop_map(
                |((hb, margin, validity), (sub_validity, msg_dur, ho_dur), plmns)| {
                    ...
                    conf.time.message.duration = nextgcore_time_from_msec(msg_dur);
                    conf.time.handover.duration = nextgcore_time_from_msec(ho_dur);
    ...
    #[allow(dead_code)]
    fn arb_app_context() -> impl Strategy<Value = NextgcoreAppContext> {
        (
    ...
    proptest! {
        ...
        fn prop_plmn_id_build_deterministic(mcc in 100u16..999, mnc in 0u16..999, mnc_len in 2u8..=3) {
            let plmn1 = NextgcorePlmnId::build(mcc, mnc, mnc_len);
            let plmn2 = NextgcorePlmnId::build(mcc, mnc, mnc_len);
        ...
        fn prop_timer_regeneration_consistent(duration_ms in 1000i64..60000) {
            let mut conf1 = NextgcoreLocalConf::new();
            let mut conf2 = NextgcoreLocalConf::new();

            conf1.time.message.duration = nextgcore_time_from_msec(duration_ms);
            conf2.time.message.duration = nextgcore_time_from_msec(duration_ms);
        ...
        fn prop_pool_size_calculation_deterministic(max_ue in 1u64..10000, max_peer in 1u64..1000) {
            ...
            let mut ctx1 = NextgcoreAppContext::new();
            let mut ctx2 = NextgcoreAppContext::new();
        ...
        fn prop_time_conversion_consistent(sec in 0i64..1000000) {
            // nextgcore_time_from_sec(n) == nextgcore_time_from_msec(n * 1000)
            prop_assert_eq!(nextgcore_time_from_sec(sec), nextgcore_time_from_msec(sec * 1000));
        }
        ...
        fn prop_pool_scales_with_ue_count(ue1 in 1u64..5000, ue2 in 5001u64..10000) {
            let mut global1 = NextgcoreGlobalConf::new();
            let mut global2 = NextgcoreGlobalConf::new();
            global1.max.ue = ue1;
            global2.max.ue = ue2;
...
"#;
        ...
        assert_eq!(conf.time.message.duration, nextgcore_time_from_msec(5000));
        assert_eq!(conf.time.handover.duration, nextgcore_time_from_msec(500));
