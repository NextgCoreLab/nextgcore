...
//! ported from lib/app/nextgcore-init.c and lib/app/nextgcore-init.h.

use crate::config::ConfigError;
use crate::context::{nextgcore_app, NextgcoreLogTs};
use crate::yaml::{NextgcoreYamlDocument, YamlError};
use std::env;
use thiserror::Error;
...
impl CommandLineOptions {
    ...
    /// Mirrors the getopt parsing in nextgcore_app_initialize()
...
pub struct NextgcoreAppInitializer {
    document: Option<NextgcoreYamlDocument>,
}

impl NextgcoreAppInitializer {
    /// Create a new initializer
    pub fn new() -> Self {
        NextgcoreAppInitializer { document: None }
    }

    /// Initialize the application
    /// Mirrors nextgcore_app_initialize()
    pub fn initialize(
        &mut self,
        version: &str,
        default_config: &str,
        ...
        nextgcore_app().set_version(version);

        // Stage 2: Load configuration file
        let config_file = opts
            .config_file
            .unwrap_or_else(|| default_config.to_string());
        nextgcore_app().set_file(&config_file);
        ...
        if let Some(log_file) = opts.log_file {
            nextgcore_app().context_mut().logger.file = Some(log_file);
        }

        if let Some(domain_mask) = opts.domain_mask {
            nextgcore_app().context_mut().logger.domain = Some(domain_mask);
        }

        if let Some(log_level) = opts.log_level {
            nextgcore_app().context_mut().logger.level = Some(log_level);
        }

        // Stage 4: Setup database URI from environment
        if let Ok(db_uri) = env::var("DB_URI") {
            nextgcore_app().set_db_uri(&db_uri);
        }

        // Stage 5: Setup config section ID
        if let Some(id) = opts.config_section_id {
            nextgcore_app().set_config_section_id(id);
        }
    ...
    /// Mirrors nextgcore_app_terminate()
    ...
    fn read_config(&mut self, file: &str) -> Result<(), InitError> {
        self.document = Some(NextgcoreYamlDocument::from_file(file)?);
        Ok(())
    }
    ...
    fn parse_config(&mut self) -> Result<(), InitError> {
        ...
        while iter.next() {
            ...
            match root_key.as_str() {
                ...
                "global" => {
                    let mut global = nextgcore_app().global_conf_mut();
                    global.parse(&mut iter)?;
                    drop(global);
                    nextgcore_app().recalculate_pool_size();
                }
                _ => {
    ...
    fn context_prepare(&self) -> Result<(), InitError> {
        ...
        nextgcore_app().context_mut().usrsctp.udp_port = 9899;

        // Prepare global configuration
        nextgcore_app().global_conf_mut().prepare();
                                        ...
                                        "timestamp" => {
                                            if let Some(child) = file_iter.recurse() {
                                                let ts = if child.bool_value() {
                                                    NextgcoreLogTs::Enabled
                                                } else {
                                                    NextgcoreLogTs::Disabled
                                                };
                                                nextgcore_app().context_mut().logger.timestamp = ts;
                                            }
                                        }
                                        _ => {}
                                    }
                            ...
                            while default_iter.next() {
                                ...
                                if default_key == "timestamp" {
                                    if let Some(child) = default_iter.recurse() {
                                        ...
                                        nextgcore_app().context_mut().logger_default.timestamp = ts;
                                    }
                                }
                            }
                        }
    ...
    pub fn document(&self) -> Option<&NextgcoreYamlDocument> {
        self.document.as_ref()
    }
}

impl Default for NextgcoreAppInitializer {
    fn default() -> Self {
        Self::new()
    }
}
...
/// Mirrors nextgcore_app_initialize()
pub fn nextgcore_app_initialize(
    version: &str,
    default_config: &str,
    args: &[String],
) -> Result<NextgcoreAppInitializer, InitError> {
    let mut initializer = NextgcoreAppInitializer::new();
...
/// Mirrors nextgcore_app_terminate()
pub fn nextgcore_app_terminate() -> Result<(), InitError> {
    let initializer = NextgcoreAppInitializer::new();
    initializer.terminate()
}

/// Read and parse configuration file
/// Mirrors nextgcore_app_config_read()
pub fn nextgcore_app_config_read(file: &str) -> Result<NextgcoreYamlDocument, InitError> {
    Ok(NextgcoreYamlDocument::from_file(file)?)
}
...
/// use nextgcore_app::init::nf_common_init;
///
...
pub fn nf_common_init(
    ...
    let _ = nextgcore_metrics::otel::init_otel(
        nextgcore_metrics::otel::OtelConfig::new(service_name).with_endpoint(otel_endpoint),
    );
...
mod tests {
    ...
    fn test_initializer_new() {
        let init = NextgcoreAppInitializer::new();
