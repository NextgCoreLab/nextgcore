...
//! ported from lib/app/nextgcore-yaml.c.
...
/// Mirrors nextgcore_yaml_iter_t from the C implementation
#[derive(Debug, Clone)]
pub struct NextgcoreYamlIter {
...
impl NextgcoreYamlIter {
    /// Initialize iterator from a YAML document root
    /// Mirrors nextgcore_yaml_iter_init()
    ...
    /// Mirrors nextgcore_yaml_iter_next()
    ...
    /// Mirrors nextgcore_yaml_iter_recurse()
    pub fn recurse(&self) -> Option<NextgcoreYamlIter> {
        match &self.root {
            Value::Mapping(map) => {
                if let Some(ref key) = self.current_key {
                    let yaml_key = Value::String(key.clone());
                    if let Some(value) = map.get(&yaml_key) {
                        return Some(NextgcoreYamlIter::new(value.clone()));
                    }
                }
                None
            }
            Value::Sequence(seq) => {
                if let Some(idx) = self.sequence_index {
                    if idx < seq.len() {
                        return Some(NextgcoreYamlIter::new(seq[idx].clone()));
                    }
                }
                None
            }
    ...
    /// Mirrors nextgcore_yaml_iter_type()
    ...
    /// Mirrors nextgcore_yaml_iter_key()
    ...
    /// Mirrors nextgcore_yaml_iter_value()
    ...
    /// Mirrors nextgcore_yaml_iter_has_value()
    ...
    /// Mirrors nextgcore_yaml_iter_bool()
...
#[derive(Debug, Clone)]
pub struct NextgcoreYamlDocument {
    root: Value,
}

impl NextgcoreYamlDocument {
    /// Parse YAML from a string
    pub fn from_str(yaml_str: &str) -> Result<Self, YamlError> {
        let root: Value =
            serde_yaml::from_str(yaml_str).map_err(|e| YamlError::ParseError(e.to_string()))?;
        Ok(NextgcoreYamlDocument { root })
    }
    ...
    pub fn iter(&self) -> NextgcoreYamlIter {
        NextgcoreYamlIter::new(self.root.clone())
    }
...
/// Macro for iterating over YAML arrays (mirrors NEXTGCORE_YAML_ARRAY_RECURSE)
#[macro_export]
...
/// Macro for iterating to next array element (mirrors NEXTGCORE_YAML_ARRAY_NEXT)
#[macro_export]
...
key3: value3
"#;
        let mut iter = NextgcoreYamlIter::from_str(yaml).unwrap();
...
"#;
        ...
        let doc2 = NextgcoreYamlDocument::from_str(&output).unwrap();
