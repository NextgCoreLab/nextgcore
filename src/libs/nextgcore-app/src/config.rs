...
//! ported from lib/app/nextgcore-config.c and lib/app/nextgcore-config.h.

use crate::yaml::{NextgcoreYamlIter, YamlNodeType};
use nextgcore_core::NextgcoreSockopt;
...
pub const NEXTGCORE_MAX_NUM_OF_BEARER: usize = 4;
/// Maximum number of SUPI ranges
pub const NEXTGCORE_MAX_NUM_OF_SUPI_RANGE: usize = 16;
/// Maximum number of GTP-U buffers per UE
pub const NEXTGCORE_MAX_NUM_OF_GTPU_BUFFER: u64 = 8;
...
/// Mirrors the parameter struct in nextgcore_app_global_conf_t
...
/// Mirrors the max struct in nextgcore_app_global_conf_t
...
/// Mirrors the sockopt struct in nextgcore_app_global_conf_t
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Mirrors nextgcore_pkbuf_config_t
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Default for PkbufConfig {
    fn default() -> Self {
        // Default values from nextgcore_pkbuf_default_init
        PkbufConfig {
...
/// Mirrors nextgcore_app_global_conf_t
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct NextgcoreGlobalConf {
...
impl NextgcoreGlobalConf {
    /// Create a new global configuration with default values
    pub fn new() -> Self {
        let mut conf = NextgcoreGlobalConf::default();
    ...
    /// Mirrors nextgcore_app_global_conf_prepare()
    ...
    /// Mirrors nextgcore_app_parse_global_conf()
    pub fn parse(&mut self, parent: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_parameter(&mut self, global_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_sockopt(&mut self, global_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_max(&mut self, global_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_pool(&mut self, global_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    /// Mirrors nextgcore_app_count_nf_conf_sections()
...
/// Convert seconds to NextgcoreTime
pub fn nextgcore_time_from_sec(sec: i64) -> NextgcoreTime {
    sec * 1_000_000
}

/// Convert milliseconds to NextgcoreTime
pub fn nextgcore_time_from_msec(msec: i64) -> NextgcoreTime {
    msec * 1_000
}
...
pub struct SbiTimeConf {
    pub client_wait_duration: NextgcoreTime,
    pub connection_deadline: NextgcoreTime,
    pub reconnect_interval: NextgcoreTime,
    pub reconnect_interval_in_exception: NextgcoreTime,
}

/// GTP message time configuration
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct GtpTimeConf {
    pub t3_response_duration: NextgcoreTime,
    pub n3_response_rcount: i32,
    pub t3_holding_duration: NextgcoreTime,
...
pub struct PfcpTimeConf {
    pub t1_response_duration: NextgcoreTime,
    pub n1_response_rcount: i32,
    pub t1_holding_duration: NextgcoreTime,
    pub n1_holding_rcount: i32,
    pub association_interval: NextgcoreTime,
    pub no_heartbeat_duration: NextgcoreTime,
}

/// Message time configuration
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct MessageTimeConf {
    pub duration: NextgcoreTime,
    pub sbi: SbiTimeConf,
    pub gtp: GtpTimeConf,
...
pub struct HandoverTimeConf {
    pub duration: NextgcoreTime,
    pub complete_delay: NextgcoreTime,
}
...
pub struct NextgcorePlmnId {
    pub mcc: [u8; 3],
    pub mnc: [u8; 3],
    pub mnc_len: u8,
}

impl NextgcorePlmnId {
    /// Build PLMN ID from MCC and MNC
    pub fn build(mcc: u16, mnc: u16, mnc_len: u8) -> Self {
        let mut plmn = NextgcorePlmnId::default();
...
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct NextgcoreSupiRange {
    pub num: usize,
    pub start: [u64; NEXTGCORE_MAX_NUM_OF_SUPI_RANGE],
    pub end: [u64; NEXTGCORE_MAX_NUM_OF_SUPI_RANGE],
}

/// Local configuration
/// Mirrors nextgcore_app_local_conf_t
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct NextgcoreLocalConf {
    pub time: TimeConf,
    pub serving_plmn_id: Vec<NextgcorePlmnId>,
}

impl NextgcoreLocalConf {
    /// Create a new local configuration with default values
    pub fn new() -> Self {
        let mut conf = NextgcoreLocalConf::default();
        conf.prepare();
        conf
    }
    ...
    pub fn prepare(&mut self) {
        ...
        self.time.message.duration = nextgcore_time_from_sec(10);

        // Handover wait duration: 300 ms (default)
        self.time.handover.duration = nextgcore_time_from_msec(300);
    ...
    pub fn regenerate_timer_durations(&mut self) {
        ...
        self.time.message.sbi.connection_deadline =
            self.time.message.sbi.client_wait_duration + nextgcore_time_from_sec(1);
        self.time.message.sbi.reconnect_interval = std::cmp::max(
            nextgcore_time_from_sec(3),
            self.time.message.sbi.client_wait_duration + nextgcore_time_from_sec(1),
        );
        self.time.message.sbi.reconnect_interval_in_exception = nextgcore_time_from_sec(2);
    ...
    /// Mirrors nextgcore_app_parse_local_conf()
    pub fn parse(&mut self, local_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_serving(&mut self, local_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
    ...
    fn parse_time(&mut self, local_iter: &mut NextgcoreYamlIter) -> Result<(), ConfigError> {
...
/// Mirrors nextgcore_app_parse_sockopt_config()
pub fn parse_sockopt_config(parent: &mut NextgcoreYamlIter) -> NextgcoreSockopt {
    let mut option = NextgcoreSockopt::default();
...
mod tests {
    ...
    fn test_global_conf_default() {
        let conf = NextgcoreGlobalConf::new();
    ...
    fn test_global_conf_validation() {
        let mut conf = NextgcoreGlobalConf::new();
    ...
    fn test_plmn_id_build() {
        let plmn = NextgcorePlmnId::build(310, 410, 3);
...
global:
  ...
  sockopt:
    ...
    linger: 5
"#;
        let doc = NextgcoreYamlDocument::from_str(yaml).unwrap();
    ...
    fn test_time_conversion() {
        assert_eq!(nextgcore_time_from_sec(1), 1_000_000);
        assert_eq!(nextgcore_time_from_msec(1), 1_000);
        assert_eq!(nextgcore_time_from_sec(10), 10_000_000);
    }
}
...
impl NextgcoreGlobalConf {
    ...
    pub fn reload(&mut self, config_path: &str) -> Result<(), ConfigError> {
        ...
        // Parse YAML document
        let doc = NextgcoreYamlDocument::from_file(config_path)
...
pub struct ConfigReloadManager {
    ...
    global_conf: Arc<Mutex<NextgcoreGlobalConf>>,
    local_conf: Arc<Mutex<NextgcoreLocalConf>>,
}
...
pub struct ConfigSnapshot {
    ...
    /// Global configuration
    pub global_conf: NextgcoreGlobalConf,
    /// Local configuration
    pub local_conf: NextgcoreLocalConf,
}
...
impl ConfigSnapshot {
    ...
    pub fn new(
        ...
        global_conf: NextgcoreGlobalConf,
        local_conf: NextgcoreLocalConf,
    ) -> Self {
        ConfigSnapshot {
            version,
            global_conf,
...
impl ConfigHistoryManager {
    ...
    pub fn rollback(
        &mut self,
        version: u64,
        current_global: &mut NextgcoreGlobalConf,
        current_local: &mut NextgcoreLocalConf,
    ) -> Result<(), ConfigError> {
...
mod version_tests {
    ...
    fn test_history_manager_snapshot() {
        ...
        let local = NextgcoreLocalConf::new();
    ...
    fn test_history_manager_rollback() {
        let mut manager = ConfigHistoryManager::new(5);
        let mut global = NextgcoreGlobalConf::new();
        let mut local = NextgcoreLocalConf::new();
...
pub struct ConfigDriftDetector {
    ...
    desired_global: NextgcoreGlobalConf,
    /// Desired/golden local configuration.
    desired_local: NextgcoreLocalConf,
...
impl ConfigDriftDetector {
    ...
    pub fn new(desired_global: NextgcoreGlobalConf, desired_local: NextgcoreLocalConf) -> Self {
        Self {
            desired_global,
            desired_local,
            scan_count: 0,
    ...
    pub fn update_baseline(&mut self, global: NextgcoreGlobalConf, local: NextgcoreLocalConf) {
    ...
    pub fn detect(
        &mut self,
        running_global: &NextgcoreGlobalConf,
        running_local: &NextgcoreLocalConf,
    ) -> DriftReport {
        self.scan_count += 1;
...
mod drift_tests {
    ...
    fn test_detect_message_duration_drift() {
        ...
        drifted_local.time.message.duration = nextgcore_time_from_sec(30);
