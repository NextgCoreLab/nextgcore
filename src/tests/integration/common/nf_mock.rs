...
use super::{CapturedMessage, MessageCapture, MessageType, NfType};
...
impl MockAmf {
    ...
    pub async fn is_ue_registered(&self, supi: &str) -> bool {
        ...
        ues.get(supi)
            .map(|ue| ue.state == UeState::Registered)
            .unwrap_or(false)
    }
}
...
mod tests {
    ...
    async fn test_generic_mock_nf() {
        ...
        let msg = CapturedMessage::new(MessageType::RegistrationRequest, Bytes::new(), "UE", "AMF");

        let response = mock.handle_message(msg).await.unwrap();
        assert!(response.is_some());
        assert_eq!(
            response.unwrap().msg_type,
