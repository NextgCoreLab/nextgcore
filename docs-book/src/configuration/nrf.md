...
- Direct mTLS to the NRF does not yet yield an identity: the rustls-verified peer certificate is available at the accept site but its URI SAN is not surfaced onto `SbiRequest` (that needs an X.509 SAN parser, which is not a dependency of this tree). Until then, mTLS-via-SCP authenticates through `trust_forwarded_client_cert` and direct connections authenticate through a signed CCA.
