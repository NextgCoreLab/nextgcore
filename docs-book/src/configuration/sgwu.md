...
- **Usage reporting (URR)** (#215, TS 29.244 §5.2.2.1 / §7.5.2.4). The SGW-U parses Create / Update / Remove URR on Session Establishment and Session Modification, stores each rule with its Measurement Method, Reporting Triggers, Volume Threshold, Volume Quota, Time Threshold and Measurement Period, links it to the PDRs that name it, and populates `PfcpSess.urr_ids`. `apply_far` counts bytes and packets against every URR the matched PDR names; crossing a requested trigger produces a Usage Report with a **monotonic UR-SEQN** (§8.2.60), carried as IE 80 in a Session Report Request with the USAR report-type bit set. Session deletion emits one final report per URR as IE 79 in the Session Deletion Response, tagged TEBUR. A threshold is checked **only when its Reporting Triggers bit is set** — §8.2.41 makes that IE the CP function's request, so a provisioned threshold with a clear trigger bit stays silent. A duration-only (`DURAT`) URR accumulates no volume, and a report that measured nothing omits the Volume Measurement IE rather than sending an all-zero one.
  - **QER-dropped and FAR-dropped traffic is NOT counted.** §5.2.2.1 measures the network resources the UP function *uses*, and a packet it discarded consumed none. Counting it would over-bill a subscriber for traffic the network refused to carry; under-counting a dropped packet costs the operator nothing it was entitled to. Traffic accepted into a BUFF FAR's buffer **is** counted, once, at buffering time — not again on flush.
  - **Nothing is sent over the wire yet.** `pfcp_path::send_pfcp_request` is a logging placeholder (see below), so a Usage Report is built correctly and then goes nowhere. Issue #217 is the missing PFCP transport; until it lands, usage reporting is verified in-process only.
