...
- **Durable state (optional, off by default).** With `--state-file` or `NEXTGCORE_PCF_STATE_FILE` set, the four primary context lists — AM policy associations, SM policy UE contexts, PDU sessions and AF application sessions — are written to a JSON snapshot on every mutation and reloaded at boot, before the SBI server accepts anything. Without it, a restart leaves the AMF and SMF holding association URIs that 404, and the SMF cannot even tear a session's policy down cleanly. Four things worth knowing operationally:
  ...
  - **Restoration signalling (#193).** The four record lists are read **per record**, so one entry whose schema moved does not discard the snapshot — but a skipped `sessions` record is an SM policy association the *SMF still holds*, and every later update or delete on it would get a bare 404. At boot, before the SBI server accepts anything, the PCF now POSTs `{notificationUri}/terminate` with a `TerminationNotification` (TS 29.512 §4.2.4) for each such association, so it is closed rather than left to rot. The `notificationUri` and `smPolicyId` are salvaged from the **raw** record — the record failed typed deserialisation, not JSON parsing, so both are still readable, and without that salvage there would be nobody to tell. Limits: the cause is `UNSPECIFIED`, because no enumerated TS 29.512 cause describes "the producer lost its own record"; delivery is fire-and-forget like every other PCF notification; a record that yields neither member is counted as unreachable and logged at error rather than silently dropped; and nothing is emitted when no state file is configured, since then nothing was restored. This is the PCF half of #193 — the SMF's session-level half is in [SMF Configuration](smf.md).
