...
- **Routing precedence** (`ScpProxy::route`, TS 29.500 §6.10.2 per code comment): a `3gpp-Sbi-Callback` notification wins first (see below); then a valid `3gpp-Sbi-Target-apiRoot` (Model C); otherwise a `3gpp-Sbi-Routing-Binding` matching a cached `3gpp-Sbi-Binding` learnt from an earlier producer response (§6.12 stickiness, with NF-set-level reselection per §5.2.3.2.6); otherwise any `3gpp-Sbi-Discovery-*` header triggers Model D. A request with none of these — or an unparsable Target-apiRoot — is rejected **400** ProblemDetails, cause `MANDATORY_IE_MISSING`.
- **Callbacks / notifications** (TS 29.500 §6.10.7 per code comment): a request carrying `3gpp-Sbi-Callback` is routed straight to its callback URI with **no discovery query and no access-token acquisition** — the destination is fixed by the consumer that registered the callback, so discovering a producer for it would address the wrong node and minting a token would be an NRF round-trip for a credential the callback target never asked for. The header outranks `Routing-Binding` and the `Discovery-*` headers; it is *not* in competition with `Target-apiRoot`, which supplies the address while the callback header decides the mode. The `3gpp-Sbi-Callback` header itself is forwarded to the target (it identifies the message as a notification), and a callback is not treated as retargeting, so no `Location` fix-up is applied.

  > **Conveyance limitation.** In practice the callback destination must come from `3gpp-Sbi-Target-apiRoot`: the shared SBI server records only the request *path*, so an absolute callback URI does not survive the hop into the SCP's routing. A callback naming no resolvable destination logs a warning and falls through to ordinary routing rather than being rejected, so a deployment whose notifications currently reach their target via the Discovery headers is not regressed. Tracked as issue #259.
