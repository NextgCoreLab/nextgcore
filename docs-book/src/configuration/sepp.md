...
- **Telescopic FQDN mapping** (#100, TS 29.573 §5.4): `GET /nsepp-telescopic/v1/mapping` on the **consumer SBI** port (not the N32 listener — it is served to NFs in this PLMN). **Exactly one** query parameter is required; neither or both is **400** `MANDATORY_IE_INCORRECT`, and any method other than GET on that path is **405**.
  - `foreign-fqdn=<fqdn>` (§5.4.2) → **200** `{telescopicLabel, seppDomain, foreignFqdn}`. The consumer forms `<telescopicLabel>.<seppDomain>`. `seppDomain` is this SEPP's own FQDN, so **`--sender` must be set** — without it the request is **500** `SYSTEM_FAILURE`, because a label with no domain is an answer the consumer cannot use.
  - `telescopic-label=<label>` (§5.4.3) → **200** with `foreignFqdn` when this SEPP issued that label, else **404** `DATA_NOT_FOUND`, which is the procedure's own "no existing mapping" answer.
  - The label is `tl` + the first 16 octets of `SHA-256(lower-cased foreign FQDN)` in hex — one 34-character DNS label for any input, as TS 23.003 §28.5.2 requires (RFC 2818 wildcard certificates cover a single subdomain level), and NOTE 1 of that clause leaves the construction implementation-specific. Deterministic and FQDN-case-insensitive, so repeated requests are idempotent.
  - Mappings are **in-memory and capped at 8192**. At the cap new ones are not stored (logged at `warn`): §5.4.2 still returns a usable telescopic FQDN, but a sibling SEPP's §5.4.3 lookup for that label answers 404. Nothing survives a restart, so a peer holding a pre-restart telescopic FQDN must re-request the mapping.
- **N32-c handshake IE corrections** (#100): the target-apiRoot capability is negotiated as `3GppSbiTargetApiRootSupported` — capital G, JSON **boolean** (TS 29.573 §6.1.5.2). It was previously emitted as a lowercase-`g` **integer** `1`, and typed `Option<i32>` on the way in, so a conformant peer's boolean `true` was a serde type error that aborted the whole `SecNegotiateReqData` parse and rejected the handshake. If you peer this SEPP with an older build of itself, the capability now reads as `false` in both directions rather than negotiating — no handshake fails, but target-apiRoot forwarding is not negotiated until both sides are upgraded.
- **`sender` on parameter exchange is optional** (#100), per `SecParamExchReqData`/`SecParamExchRspData` (`required: [n32fContextId]` only). When omitted, the peer is resolved from `senderApiRoot`'s host, or from the single N32-c handshake in flight; with two in flight and no other identifier the request is **400**, because guessing would derive the N32-f key hierarchy against the wrong peer's exporter secret and yield a context that establishes and then cannot decrypt. A `sender` naming a third party on an established association is still refused as an FQDN mismatch.
