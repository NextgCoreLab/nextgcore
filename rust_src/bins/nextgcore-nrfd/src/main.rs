...
use clap::Parser;
use nextgcore_nrfd::{
    nrf_context_final, nrf_context_init, nrf_sbi_close, nrf_sbi_open, NrfSmContext, SbiServerConfig,
};
...
fn main() -> Result<()> {
    ...
    // Main event loop
    run_event_loop(&mut nrf_sm, shutdown)?;
...
/// Main event loop
fn run_event_loop(_nrf_sm: &mut NrfSmContext, shutdown: Arc<AtomicBool>) -> Result<()> {
